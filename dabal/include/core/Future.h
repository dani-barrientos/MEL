...
// no sé cómo encajar el unknownerror
	...
	/**
	* @class Future
	*  Represents a value that *maybe* is not present at the current moment. This value will be generated by *someone* and will be available at "some moment"
	* The value is retrieved using various ways:
	*	- pooling in a loop checking for "getValid" and "error"
	*	- using wait functions, as ::utilities::waitForFutureMThread or ::core::waitForFutureThread
	*   - callback notification with subcribeCallback
	* should check getValid, in which case there is an error ( see error )
	*/
	template <typename T>
	class Future 
	: public _private::Future_Common<T>
	{
	public:

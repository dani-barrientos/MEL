...
#define SYNCHRONIZED( TRet, function_name,args,runnable) \
	...
	TRet function_name( MAKE_PARAMS args  ){ \
		...
		if ( result.getError()->error == Runnable::ERRORCODE_EXCEPTION ){\
				...
				throw ::core::UnknownException();\
		}else return result.getValue();}
//@todo lo que viene es ñapa por que lo necesito ya,para funciones static. Lo pensaré mejor más adelante
...
/*#define POSTED( TRet, function_name,args,runnable,defaultvalue) \
	...
	TRet function_name( MAKE_PARAMS args  ){ \
	runnable->post( RUNNABLE_CREATETASK( ::mpl::returnAdaptor<void>( CREATE_LINKER( void,function_name##_sync,args ),true ) ); \
	return defaultvalue;}
*/
#define POSTED( TRet, function_name,args,runnable,defaultvalue) \
	void function_name##_sync args ; \
...
//runnable->execute<void>(  CREATE_LINKER( void,makeMemberEncapsulate( &MyType::function_name##_sync,this),args ) ); 
#define POSTED_METHOD( TRet, function_name,args,runnable,defaultvalue) \
	void function_name##_sync args ; \
	TRet function_name( MAKE_PARAMS args  ){ \
	runnable->post( RUNNABLE_CREATETASK( ( ::mpl::returnAdaptor<void>( CREATE_LINKER( void, makeMemberEncapsulate(&MyType::function_name##_sync,this),args),true ) )) ); \
	return defaultvalue;}

//CREATE_LINKER( TRet,function_name##_sync,args ) ; \

// 	TRet function_name( GET_TYPES args  ){ \
// 	}

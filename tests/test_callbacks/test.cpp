...
{
    if ( sCurrentTest )
        sCurrentTest->addTextToBuffer( std::move( str ),
...
typedef std::pair<
    Int2Type<0>, CallbackSubscriptor<::mel::core::CSNoMultithreadPolicy, float>>
    CS1;
typedef std::pair<Int2Type<1>, CallbackSubscriptor<
                                   ::mel::core::CSNoMultithreadPolicy, float&>>
    CS2;
class Pepe : private CS1, private CS2
{
  public:
    template <class T> auto subscribe1( T&& f )
    {
        return CS1::second.subscribeCallback( std::forward<T>( f ) );
    }
    template <class T> auto subscribe2( T&& f )
    {
        return CS2::second.subscribeCallback( std::forward<T>( f ) );
    }
    template <class T> auto unsubscribe1( T&& f )
    {
        return CS1::second.unsubscribeCallback( std::forward<T>( f ) );
    }
    template <class T> auto unsubscribe2( T&& f )
    {
        return CS2::second.unsubscribeCallback( std::forward<T>( f ) );
    }
    auto trigger1( float a ) { return CS1::second.triggerCallbacks( a ); }
    auto trigger2( float a ) { return CS2::second.triggerCallbacks( a ); }
};
...
int TestCallbacks::onExecuteTest()
{
    ...
    pp.subscribe1( f );
    int s1 = pp.subscribe1(
        std::function<::mel::core::ECallbackResult( float )>( f1 ) );
    ...
    pp.subscribe2(
        std::function<::mel::core::ECallbackResult( float& )>( f2 ) );
    ...
    pp.trigger1( 5 );
    sCurrentTest->checkOccurrences( "f1", 2, __FILE__, __LINE__,
                                    tests::BaseTest::LogLevel::Info );
    sCurrentTest->checkOccurrences( "lambda1", 1, __FILE__, __LINE__,
                                    tests::BaseTest::LogLevel::Info );
    sCurrentTest->clearTextBuffer();
    pp.trigger1( 5 );
    sCurrentTest->checkOccurrences( "f1", 2, __FILE__, __LINE__,
                                    tests::BaseTest::LogLevel::Info );
    sCurrentTest->checkOccurrences( "lambda1", 0, __FILE__, __LINE__,
                                    tests::BaseTest::LogLevel::Info );
    pp.unsubscribe1( s1 );
    ...
    sCurrentTest->clearTextBuffer();
    pp.trigger1( 5 );
    sCurrentTest->checkOccurrences( "f1", 1, __FILE__, __LINE__,
                                    tests::BaseTest::LogLevel::Info );
    pp.unsubscribe1( f1 );
    sCurrentTest->clearTextBuffer();
    pp.trigger1( 5 );
    sCurrentTest->checkOccurrences( "f1", 0, __FILE__, __LINE__,
                                    tests::BaseTest::LogLevel::Info );

    // Second subscriptor
    sCurrentTest->clearTextBuffer();
    pp.trigger2( 6 );
    sCurrentTest->checkOccurrences( "f2", 1, __FILE__, __LINE__,
                                    tests::BaseTest::LogLevel::Info );
    sCurrentTest->checkOccurrences( "lambda2", 1, __FILE__, __LINE__,
                                    tests::BaseTest::LogLevel::Info );
    sCurrentTest->checkOccurrences( "lambda3", 1, __FILE__, __LINE__,
                                    tests::BaseTest::LogLevel::Info );
    sCurrentTest->clearTextBuffer();
    pp.trigger2( 6 );
    sCurrentTest->checkOccurrences( "f2", 1, __FILE__, __LINE__,
    ...
    sCurrentTest->checkOccurrences( "lambda2", 1, __FILE__, __LINE__,
                                    tests::BaseTest::LogLevel::Info );
    sCurrentTest->checkOccurrences( "lambda3", 0, __FILE__, __LINE__,
                                    tests::BaseTest::LogLevel::Info );
...
int test_callbacks::test()
{
    ...
    CallbackSubscriptor<::mel::core::CSMultithreadPolicy,int,float,float,int,int,float,int>
cs4;
    ...
    cs2.subscribeCallback( std::function<
mel::core::ECallbackResult(int,float)>(f2));
  //  cs2.unsubscribeCallback( std::function<
mel::core::ECallbackResult(int,float)>(f2)); cs2.triggerCallbacks(2,4.5f);
...
void TestCallbacks::registerTest()
{
    TestManager::getSingleton().registerTest(
        TEST_NAME, "callbacks and callbacksubscriptor tests",
        std::make_unique<TestCallbacks>() );
}

int TestCallbacks::onExecuteAllTests() { return executeTest(); }

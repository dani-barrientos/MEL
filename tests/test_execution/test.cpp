...
/*
example code to detect MSVC compile error in some version related with
/permissive- and /Zc:lambda flags struct MyClass
{
    //no working in MSVC
    template <class TArg,class F> void test(F&& f,TArg&& arg) noexcept(
sizeof(TArg)>0)
    {
...
struct TestClass
{
  ...
  public:
    TestClass( tests::BaseTest::LogLevel ll = tests::BaseTest::LogLevel::Info,
               bool atb = true )
        : val( 1 ), logLevel( ll ), addToBuffer( atb )
    {
    ...
    explicit TestClass(
        ...
        bool atb = true )
        : val( aVal ), logLevel( ll ), addToBuffer( atb )
    {
        _printTxt( "TestClass constructor\n" );
    }
    ...
    ~TestClass() { _printTxt( "TestClass destructor\n" ); }
    ...
    void setLogLevel( tests::BaseTest::LogLevel ll ) { logLevel = ll; }
...
void _basicTests( ExecutorType ex, ThreadRunnable* th, tests::BaseTest* test )
{
    core::Event event;
    TestClass pp( 8 );
    ...
    th->fireAndForget(
        [ex, &event, test, &pp, &vec]() mutable
        {
            srand( time( NULL ) );
            constexpr tests::BaseTest::LogLevel ll =
                tests::BaseTest::LogLevel::Debug;
            ...
            constexpr int initVal = 8;
            {
                ...
                auto res1_1 =
                    ...
                    // })
                    execution::start( ex ) |
                    mel::execution::inmediate(
                        TestClass( initVal, ll ) ) // move constructor ¿@why 3?
                    | mel::execution::parallel(
                          []( TestClass v ) noexcept // one copy and "some"
                                                     // moves ¿why?
                          {
                              text::debug( "Bulk 1" );
                              auto prevVal = v.val;
                              ...
                              v.val =
                                  prevVal +
                                  5; // should has effect, for testing purposes
                          },
                          []( const TestClass& v )
                          {
                              text::debug( "Bulk 2" );
                    ...
                    mel::execution::transfer(
                        ex2 ) // transfer execution to a different executor
                    | mel::execution::next(
                          [test, ll]( const TestClass& v ) noexcept
                          {
                              // text::info("Val = {}",v.value().val);
                              const_cast<TestClass&>( v ).val +=
                                  3; // very bad practice. It will change passed
                                     // value, For testing purposes
                              // v.val+= 3;
                              return v; //@why a lot of moves after here, maybe
                                        // because of the on_all
                          } );
                ...
                try
                {
                    auto res1 = mel::tasking::waitForFutureMThread(
                        mel::execution::on_all( ex, res1_1, res1_2 ) );
                    auto& finalResult = res1.value();
                    int finalVal = std::get<0>( finalResult ).val;
                    int expectedVal =
                        initVal + 3; // because the "forced" const_cast
                    text::debug( "Finish Val = {}", finalVal );
                    std::stringstream ss;
                    if ( finalVal != expectedVal )
                    {
                        ss << " final value after chain of execution is no "
                              "correct. Expected "
                           << expectedVal << " got " << finalVal;
                    }
                    if ( std::get<1>( finalResult ) != strR )
                    {
                        ss << " final second element in value after chain of "
                              "execution is no correct. Expected "
                           << strR << " got " << std::get<1>( finalResult );
                    }
            ...
            test->checkOccurrences(
                "TestClass constructor", 2, __FILE__, __LINE__,
                tests::BaseTest::LogLevel::
                    Info ); // initial constructor from inmedaite, and default
                            // constructor in tuple elements
            test->checkOccurrences( "TestClass copy", 2, __FILE__, __LINE__,
                                    tests::BaseTest::LogLevel::Info );
            test->checkOccurrences( "destructor",
                                    test->findTextInBuffer( "constructor" ),
                                    __FILE__, __LINE__ );
            ...
            pp.setLogLevel( ll );
            {
                ...
                try
                {
                    auto res2 = mel::tasking::waitForFutureMThread(
                        execution::launch(
                            ex,
                            []( TestClass& tc ) noexcept -> TestClass&
                            {
                                tc.val = 7;
                                tasking::Process::wait( 1000 );
                                return tc;
                            ...
                            std::ref( pp ) ) |
                        mel::execution::next(
                            [test, ll]( TestClass& v ) noexcept -> TestClass&
                            {
                                v.val += 10;
                                return v;
                            } ) );
            ...
            test->checkOccurrences( "constructor", 0, __FILE__, __LINE__,
                                    tests::BaseTest::LogLevel::Info );
            test->checkOccurrences( "destructor",
                                    test->findTextInBuffer( "constructor" ),
                                    __FILE__, __LINE__ );
            // now a little more complex test //TODO: meter al final un throw o
            // algo así
            text::info( "A little more complex functor chaining using "
                        "reference and parallel loops" );
            test->clearTextBuffer();
...
#define INITIAL_VALUE 2
            try
            {
                auto res3 = mel::tasking::waitForFutureMThread(
                    ...
                    mel::execution::next(
                        [test, ll]( vector<TestClass>& v ) noexcept
                        -> vector<TestClass>&
                        {
                            // fill de vector with a simple case for the result
                            // to be predecible I don't want out to log the
                            // initial constructions, oncly constructons and
                            // after this function
                            auto t = TestClass( INITIAL_VALUE,
                                                tests::BaseTest::LogLevel::None,
                                                false );
            ...
            test->checkOccurrences( "constructor", 0, __FILE__, __LINE__,
                                    tests::BaseTest::LogLevel::Info );
            test->checkOccurrences( "copy", 0, __FILE__, __LINE__,
                                    tests::BaseTest::LogLevel::Info );

            test->checkOccurrences(
                std::to_string( ( INITIAL_VALUE + 1 ) * 2 + 5 + 1 ),
                vec.size() / 2, __FILE__, __LINE__,
                tests::BaseTest::LogLevel::Info );
            test->checkOccurrences(
                std::to_string( ( INITIAL_VALUE + 1 ) * 3 + 5 + 1 ),
                vec.size() / 2, __FILE__, __LINE__,
                tests::BaseTest::LogLevel::Info );
...
void _testMeanVector(
    ::mel::execution::ExFuture<ExecutorType, vector<double>> fut, string title,
    tests::BaseTest* test )
{
    ...
    try
    {
        ...
        auto res5 = mel::core::waitForFutureThread(
            ...
            {
                    //int vecSize = std::rand()%1000'000; //generate random big
            vector constexpr int vecSize = 2000'000; v.resize(vecSize); for(
            size_t i = 0; i < vecSize;++i) v[i] = (std::rand()%20)/3.f; //to
            create a float return v;
            },std::ref(*values))
            */
            fut
            //|
            // mel::execution::parallel_convert<std::tuple<double,double,double,double>>(
            ////calculate mean in 4 parts @todo ¿cómo podrái devolver este
            // resultado a siguiente funcion?
            ...
            mel::execution::next(
                []( const std::tuple<double, double, double, double>& means )
                {
                    return ( std::get<0>( means ) + std::get<1>( means ) +
                             std::get<2>( means ) + std::get<3>( means ) );
                } ) );
        uint64_t t1 = timer.getMilliseconds();
        text::info( "Mean = {}. Time spent = {} seconds", res5.value(),
                    (float)( ( t1 - t0 ) / 1000.f ) );
        tests::BaseTest::addMeasurement( title + " time:",
                                         (float)( ( t1 - t0 ) / 1000.f ) );
    }
    catch ( std::exception& e )
    {
    ...
    tests::BaseTest* test )
{
    ...
    try
    {
        Timer timer;
        uint64_t t0 = timer.getMilliseconds();
        int numParts = ExecutorTraits<Executor<ExecutorType>>::has_parallelism
                           ? mel::core::getNumProcessors()
                           : 1;
        ...
        uint64_t t1 = timer.getMilliseconds();
        text::info( "Using loop. Mean = {}. Time spent = {} seconds",
                    res5.value(), (float)( ( t1 - t0 ) / 1000.f ) );
        tests::BaseTest::addMeasurement( title + " time:",
                                         (float)( ( t1 - t0 ) / 1000.f ) );
    }
    catch ( std::exception& e )
...
int _testLaunch( tests::BaseTest* test )
{
    ...
    mel::text::info( "Test execution: launch" );
    {
        ...
        /*
        InlinExecutor doesn't do any copy because it's customized for this
        specific case
        {
                auto th1 = ThreadRunnable::create(true);
...
// "move" tests are removed because they depend greately on compiler and debug
// vs release
template <class ExecutorType>
void _testCapturesHelper( ExecutorType ex, ThreadRunnable* th,
                          tests::BaseTest* test )
{
#define INIT_VALUE 2
    sCurrentTest = test;
    tests::BaseTest::LogLevel ll = tests::BaseTest::LogLevel::Debug;
    {
        ...
        // first test: Passing lambda as rvalue reference and capturing object
        // by copy
        TestClass pp( INIT_VALUE, ll );
        int cont = INIT_VALUE;
        auto res = mel::core::waitForFutureThread( execution::launch(
            ex,
            [pp, cont]()
            {
                mel::text::info( "Launch before wait. pp.val = {}, cont = {}",
                                 pp.val, cont );
                if ( cont != INIT_VALUE )
                {
                    stringstream ss;
                    ss << "Cont value is " << cont << " but should be "
                       << INIT_VALUE << '\n';
                ...
                {
                    stringstream ss;
                    ss << "pp.val value is " << pp.val << " but should be "
                       << INIT_VALUE << '\n';
                    sCurrentTest->setFailed( ss.str() );
                }
                if ( mel::execution::ExecutorTraits<
                         ExecutorType>::has_microthreading )
                    mel::tasking::Process::wait( 2000 );
                else
                    mel::text::warn(
                        "Current executor hasn't microthread capabilities" );
                mel::text::info( "Launch after wait. pp.val = {}, cont = {}",
                                 pp.val, cont );
                return "HOLA"s;
            } ) );
        // mel::text::info("Value = {}",res.value());
        test->checkOccurrences( "TestClass constructor", 1, __FILE__, __LINE__,
                                tests::BaseTest::LogLevel::Info );
        test->checkOccurrences( "TestClass copy", 1, __FILE__, __LINE__,
                                tests::BaseTest::LogLevel::Info );
        // test->checkOccurrences("TestClass
        // move",2,__FILE__,__LINE__,tests::BaseTest::LogLevel::Info);  //moved
        // when calling launch + move to execute
    ...
    }
    {
        ...
        // second test: Passing lambda as lvalue reference and capturing object
        // by copy
        TestClass pp( INIT_VALUE, ll );
        int cont = INIT_VALUE;
        auto lmb = [pp, cont]()
        ...
        auto res =
            mel::core::waitForFutureThread( execution::launch( ex, lmb ) );
        //		mel::text::info("Value = {}",res.value());
        test->checkOccurrences( "TestClass constructor", 1, __FILE__, __LINE__,
                                tests::BaseTest::LogLevel::Info );
        test->checkOccurrences(
            "TestClass copy", 2, __FILE__, __LINE__,
            tests::BaseTest::LogLevel::Info ); // copy in capture + copy lamda
        //	test->checkOccurrences("TestClass
        // move",1,__FILE__,__LINE__,tests::BaseTest::LogLevel::Info); //moved
        // when executed
    }
    // test next
    {
        ...
        auto res = mel::core::waitForFutureThread(
            execution::launch( ex, []() noexcept { return "Dani"s; } ) |
            execution::next(
                [pp, cont]( const string& s )
                {
                    mel::text::info( "Next before wait. pp.val = {}, cont = {}",
                                     pp.val, cont );
                    if ( cont != INIT_VALUE )
                    {
                        stringstream ss;
                    ...
                    if ( mel::execution::ExecutorTraits<
                        ...
                        mel::text::warn( "Current executor hasn't microthread "
                                         "capabilities" );
                    mel::text::info( "Next after wait. pp.val = {}, cont = {}",
                                     pp.val, cont );
                    return s;
                } )
        ...
        test->checkOccurrences( "TestClass constructor", 1, __FILE__, __LINE__,
                                tests::BaseTest::LogLevel::Info );
        test->checkOccurrences(
            "TestClass copy", 1, __FILE__, __LINE__,
            tests::BaseTest::LogLevel::Info ); // copy in capture
        //	test->checkOccurrences("TestClass
        // move",10,__FILE__,__LINE__,tests::BaseTest::LogLevel::Info); //moved
        // when executed
    }
    {
        sCurrentTest->clearTextBuffer();
        ...
        {
            mel::text::info( "Next before wait. pp.val = {}, cont = {}", pp.val,
                             cont );
            if ( cont != INIT_VALUE )
            {
                stringstream ss;
            ...
            if ( mel::execution::ExecutorTraits<
                ...
                mel::text::warn(
                    "Current executor hasn't microthread capabilities" );
            mel::text::info( "Next after wait. pp.val = {} cont = {}", pp.val,
                             cont );
            return s;
        };
        auto res = mel::core::waitForFutureThread(
        ...
        test->checkOccurrences(
            ...
            tests::BaseTest::LogLevel::Info ); // copy in capture
        //		test->checkOccurrences("TestClass
        // move",7,__FILE__,__LINE__,tests::BaseTest::LogLevel::Info); //moved
        // when executed
    }
    // parallel
    {
        ...
        auto res = mel::core::waitForFutureThread(
            ...
            execution::parallel(
                ...
                [pp, cont]( const string& s )
                {
                    mel::text::info( "Parallel t2. pp.val = {} cont = {}",
                                     pp.val, cont );
                    if ( cont != INIT_VALUE )
                    {
                        stringstream ss;
        ...
        test->checkOccurrences(
            ...
            tests::BaseTest::LogLevel::Info ); // copy in capture
        //	test->checkOccurrences("TestClass
        // move",8,__FILE__,__LINE__,tests::BaseTest::LogLevel::Info); //moved
        // when executed
    }
    {
        sCurrentTest->clearTextBuffer();
        ...
        auto lmb = [pp, cont]( const string& s )
        {
            mel::text::info( "Parallel lmb. pp.val = {} cont = {}", pp.val,
                             cont );
            if ( cont != INIT_VALUE )
            {
                stringstream ss;
        ...
        //	mel::text::info("Value = {}",res.value());
        test->checkOccurrences(
            "TestClass constructor", 1, __FILE__, __LINE__,
            tests::BaseTest::LogLevel::Info ); // initial constructor from
                                               // inmedaite, and default
                                               // constructor in tuple elements
        test->checkOccurrences( "TestClass copy", 2, __FILE__, __LINE__,
                                tests::BaseTest::LogLevel::Info );
        // test->checkOccurrences("destructor",test->findTextInBuffer("constructor"),__FILE__,__LINE__);
    ...
    // parallel_convert rvalue ref
    {
        ...
        auto res = mel::core::waitForFutureThread(
            ...
            execution::parallel_convert(
                ...
                {
                    mel::text::info(
                        "Parallel_convert t2. pp.val = {} cont = {}", pp.val,
                        cont );
                    if ( cont != INIT_VALUE )
                    {
                        stringstream ss;
        ...
        //	mel::text::info("Value = ({}
        //{})",std::get<0>(res.value()),std::get<1>(res.value()));
        test->checkOccurrences(
            "TestClass constructor", 1, __FILE__, __LINE__,
            tests::BaseTest::LogLevel::Info ); // initial constructor from
                                               // inmedaite, and default
    ...
    // parallel_convert lvalue ref
    {
        ...
        auto lmb = [pp, cont]( const string& s )
        {
            mel::text::info( "Parallel_convert lmb. pp.val = {} cont = {}",
                             pp.val, cont );
            if ( cont != INIT_VALUE )
            {
                stringstream ss;
    ...
    // loop
    {
        ...
        test->checkOccurrences(
            ...
            tests::BaseTest::LogLevel::Info ); // initial constructor from
                                               ...
                                               // constructor in tuple elements
        test->checkOccurrences(
            "TestClass copy", 11, __FILE__, __LINE__,
            tests::BaseTest::LogLevel::Info ); // each iteration means a copy of
                                               // the lambda
        // test->checkOccurrences("destructor",test->findTextInBuffer("constructor"),__FILE__,__LINE__);
    }
    // loop
    {
        ...
        test->checkOccurrences(
            ...
            tests::BaseTest::LogLevel::Info ); // initial constructor from
                                               // inmedaite, and default
                                               // constructor in tuple elements
        test->checkOccurrences( "TestClass copy", 12, __FILE__, __LINE__,
                                tests::BaseTest::LogLevel::Info );
        // test->checkOccurrences("destructor",test->findTextInBuffer("constructor"),__FILE__,__LINE__);
    }
}
...
int _testCaptures( tests::BaseTest* test )
{
    ...
    /*
    removed inline executor from this test because it's optimized and number of
    copies is less
    {
            auto th1 = ThreadRunnable::create(true);
            execution::NaiveInlineExecutor ex;
...
int _testAdvanceSample( tests::BaseTest* test )
{
    ...
    auto initFut = execution::launch( exr,
                                      []()
                                      {
                                          ...
                                          for ( size_t i = 0; i < vecSize; ++i )
                                              v[i] = ( std::rand() % 20 ) /
                                                     3.0; // to create a double
                                          return v;
                                      } );
    core::waitForFutureThread(
        initFut ); // wait for completion of vector creation to not interfere in
                   // time measurement
    auto initRes =
        core::waitForFutureThread<mel::core::WaitErrorNoException>( initFut );
    Timer timer;
    uint64_t t0 = timer.getMilliseconds();
    ...
    uint64_t t1 = timer.getMilliseconds();
    mel::text::info( "Mean = {}. Time spent = {}", mean,
                     (float)( ( t1 - t0 ) / 1000.f ) );
    tests::BaseTest::addMeasurement( "vector mean: plain way time:",
                                     (float)( ( t1 - t0 ) / 1000.f ) );
    {
        parallelism::ThreadPool::ThreadPoolOpts opts;
        ...
        _testMeanVector( execution::transfer( initFut, extp ),
                         "vector mean: ThreadPoolExecutor", test );
    }
    {
        exr.setOpts( { true, false } );
        text::info( "vector mean: RunnableExecutor" );
        _testMeanVector(
            execution::transfer( initFut, exr ),
            "vector mean: RunnableExecutor",
            test ); // the transfer is not neccesary because initFut is launched
                    // in exr, but jsut in case it changes
    }
    {
        // en GCC 9 esto no compila, tiene algún bug en resolucion de overloads
        // con templates
        execution::InlineExecutor ex;
        // execution::NaiveInlineExecutor ex;
        text::info( "vector mean: InlineExecutor" );
        _testMeanVector( execution::transfer( initFut, ex ),
                         "vector mean: InlineExecutor", test );
    }
    // now testing different algorithm using execution ::loop
    {
        ...
        _testMeanVectorLoop( execution::transfer( initFut, extp ),
                             "vector mean(loop): ThreadPoolExecutor", test );
    }
    {
        exr.setOpts( { true, false } );
        text::info( "vector mean(loop): RunnableExecutor" );
        _testMeanVectorLoop( execution::transfer( initFut, exr ),
                             "vector mean(loop): RunnableExecutor", test );
    }
    {
        execution::InlineExecutor ex;
        text::info( "vector mean(loop): InlineExecutor" );
        _testMeanVectorLoop( execution::transfer( initFut, ex ),
                             "vector mean(loop): InlineExecutor", test );
    }
...
#endif
// don't want to optimize this function because i want to measure in same
// conditions for MSVC
#ifdef _MSC_VER
#pragma optimize( "", off )
#endif
template <class F>
void _measureTest( string txt, F f, size_t iters,
                   size_t loopSize ) OPTIMIZE_FLAGS;
template <class F>
void _measureTest( string txt, F f, size_t iters, size_t loopSize )
{
    int mean = 0;
    Timer timer;
...
int _testFor( tests::BaseTest* test )
{
    ...
    blockingOpts.schedulerOpts =
        mel::tasking::ProcessScheduler::BlockingOptions{};
    Runnable::RunnableCreationOptions lockFreeOpts;
    lockFreeOpts.schedulerOpts =
        mel::tasking::ProcessScheduler::LockFreeOptions{
            10000, 4 }; // big initial buffer
    _measureTest(
        "Runnable executor with independent tasks",
        [loopSize, opts = blockingOpts]
        {
            ...
            ex.setOpts( exopts );
            core::waitForFutureThread( execution::loop(
                execution::start( ex ),
                [loopSize]() {
        ...
        iters, loopSize );
    _measureTest(
        "Runnable executor with independent tasks, Lock-free",
    ...
    _measureTest(
        "Runnable executor with independent tasks pausing thread",
        ...
        iters, loopSize );
    _measureTest(
        "Runnable executor WITHOUT independent tasks",
        [loopSize, opts = blockingOpts]()
        {
            auto th1 = ThreadRunnable::create( true, opts );
            execution::Executor<Runnable> ex( th1 );
        ...
        iters, loopSize );
    _measureTest(
        "ThreadPool executor, grouped tasks",
        [loopSize]()
        {
            parallelism::ThreadPool::ThreadPoolOpts opts;
        ...
        iters, loopSize );
    _measureTest(
        "ThreadPool executor, grouped tasks, lock-free",
        [loopSize]()
        {
            parallelism::ThreadPool::ThreadPoolOpts opts;
            opts.threadOpts.schedulerOpts = ProcessScheduler::LockFreeOptions{};
...
#if USE_SPDLOG
                spdlog::warn(
                    "Test number {} doesn't exist. Executing default test", n );
#endif
                result = _testFor( this );
            }
        }
...
{
    TestManager::getSingleton().registerTest(
        TEST_NAME,
        "execution tests:\n - 0 = mono thread;\n - 1 = performance launching a "
        "bunch of tasks",
        std::make_unique<TestExecution>() );
}

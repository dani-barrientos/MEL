...
class Hija : public Base
{
  public:
    ...
    {
        // because every object shares same vector and generator, it will hurts
...
{
  public:
    TestResult( string name, size_t nThreads, unsigned int baseTime )
        : mBaseTime( baseTime ), mName( name ), mData( nThreads )
    {
    }
    void addTime( int nThread, unsigned int time )
    {
        mData[nThread - 1] = time;
    }
...
int TestParallelism::onExecuteTest()
{
    ...
    mel::text::info( "Running test from 1 thread to {}. Number of cores is {}",
                     maxThreads, numCores );
    ...
    auto test = std::make_unique<TestResult>( PARALLEL_ARRAY_INDEXED,
                                              maxThreads, elapsed );
    results[test->getName()] = std::move( test );
    test = std::make_unique<TestResult>( PARALLEL_ARRAY_ITERATOR, maxThreads,
                                         elapsed );
    results[test->getName()] = std::move( test );
    ...
    test = std::make_unique<TestResult>( PARALLEL_VECTOR_INDEXED, maxThreads,
                                         elapsed );
    results[test->getName()] = std::move( test );
    test = std::make_unique<TestResult>( PARALLEL_VECTOR_ITERATOR, maxThreads,
                                         elapsed );
    results[test->getName()] = std::move( test );
    t0 = timer.getMilliseconds();
    for ( int i = 0; i < tries; ++i )
    ...
    test = std::make_unique<TestResult>( PARALLEL_LIST_ITERATOR, maxThreads,
                                         elapsed );
    results[test->getName()] = std::move( test );
    ...
    for ( int i = 1; i <= maxThreads; ++i )
    {
        ...
        for ( int i = 0; i < tries; ++i )
            ::mel::core::waitForBarrierThread(
                ::parallelism::_for( &myPool, exopts, begin, end,
                                     [&objs]( int idx )
                                     {
                                         objs[idx]->f();
        ...
        elapsed = timer.getMilliseconds() - t0;
        mel::text::info( "Parallel method indexing (array). Time: {}",
                         elapsed );
        results[PARALLEL_ARRAY_INDEXED]->addTime( i, elapsed );
        t0 = timer.getMilliseconds();
        for ( int i = 0; i < tries; ++i )
            ::mel::core::waitForBarrierThread(
                ::mel::parallelism::_for( &myPool, exopts, begin, end,
                                          [&objs2]( int idx )
                                          {
                                              objs2[idx]->f();
                                              //  mel::text::debug("it {}",idx);
        ...
        elapsed = timer.getMilliseconds() - t0;
        mel::text::info( "Parallel method indexing (vector). Time: {}",
                         elapsed );
        results[PARALLEL_VECTOR_INDEXED]->addTime( i, elapsed );
        t0 = timer.getMilliseconds();
        for ( int i = 0; i < tries; ++i )
            ::mel::core::waitForBarrierThread( ::mel::parallelism::_for(
                &myPool, exopts, objs.begin(), objs.end(),
                []( decltype( objs )::iterator i )
                {
                    ( *i )->f();
                    //  mel::text::debug("it {}",idx);
                } ) );
        elapsed = timer.getMilliseconds() - t0;
        mel::text::info( "Parallel method with iterators (array). Time: {}",
                         elapsed );
        results[PARALLEL_ARRAY_ITERATOR]->addTime( i, elapsed );
        t0 = timer.getMilliseconds();
        for ( int i = 0; i < tries; ++i )
            ::mel::core::waitForBarrierThread( ::mel::parallelism::_for(
                &myPool, exopts, objs2.begin(), objs2.end(),
                []( decltype( objs2 )::iterator i ) { ( *i )->f(); } ) );
        elapsed = timer.getMilliseconds() - t0;
        mel::text::info( "Parallel method with iterators (vector). Time: {}",
                         elapsed );
        ...
        for ( int i = 0; i < tries; ++i )
            ::mel::core::waitForBarrierThread( ::mel::parallelism::_for(
                &myPool, exopts, objs3.begin(), objs3.end(),
                []( decltype( objs3 )::iterator i ) { ( *i )->f(); } ) );
        elapsed = timer.getMilliseconds() - t0;
        mel::text::info( "Parallel method with iterators (list). Time: {}",
                         elapsed );
        ...
        // for(int i = 0; i < tries; ++i )
        //     For(&myPool,exopts,objs4.begin(),objs4.end(),[](decltype(objs4)::iterator
        //     i  )
        //         {
        //             (*i).f();
        //         }).wait();
        // spdlog::info("Parallel method with iterators plain objects (list).
        // Time: {}",timer.getMilliseconds()-t0);
    }
    // show results
    ...
    for ( const auto& result : results )
    {
        ...
        {
            str << element.second * 100.f / test->getBaseTime() << "% ("
                << element.first + 1 << ") "; // number of threads is plus 1
        }
        mel::text::info( str.str() );
        mel::text::info( "      Best time = {}% for {} threads",
                         min * 100.f / test->getBaseTime(), minIdx + 1 );
        mel::text::info( "      Worst time = {}% for {} threads",
                         max * 100.f / test->getBaseTime(), maxIdx + 1 );
    }
...
void TestParallelism::registerTest()
{
    TestManager::getSingleton().registerTest(
        TEST_NAME, "parallelism tests", std::make_unique<TestParallelism>() );
}
int TestParallelism::onExecuteAllTests()
{
    // return executeTest(); //same tests as individual
    return 0; //@todo todavái no tengo estos testes bien montados. De todos
              // modos los testes de execution son más genéricos
}

...
{
  public:
    // testing custom operator new. Now do the default but could have my own
    // pool
...
mel::tasking::EGenericProcessResult staticFuncTask( RUNNABLE_TASK_PARAMS,
                                                    int& var )
{
    ++var;
...
static Timer sTimer;
/**
 * objetivo: muchas tareas concurrentes pero que al final un resultado no debe
 * ser modificado, por lo que el test se pasará si el valor sigue manteniendose
 */
...
static int _testsDebug( tests::BaseTest* test )
{
        ...
        th1->fireAndForget(
            []() noexcept
            {
                ...
                auto fut = th->execute<int>(
                    []()
                    {
                        // pensar también como encajarái esto con establecer el
                        // error de otra forma continuar ejemplo generando otro
                        // error distinto que no sea timeout throw
                        // std::runtime_error("PRUEBAAAA");
                ...
                if ( waitWithException )
                {
                    try
                    {
                        auto res = ::mel::tasking::waitForFutureMThread<
                            ...
                            fut, 500 ); // will wait only for 20 msecs
                        mel::text::info( "Result = {}", res.value() );
                    }
                    catch ( mel::core::WaitException& e )
                    {
                        mel::text::error( "Error waiting. Code {}, Reason = {}",
                                          (int)e.getCode(), e.what() );
                    }
                    catch ( std::exception& e )
                    {
                        mel::text::error(
                            "Error waiting for result. Reason = {}", e.what() );
                    }
                    catch ( string& msg )
                    {
                        mel::text::error( "Error waiting for result. Msg = {}",
                                          msg );
                    }
                    catch ( ... )
                    {
                ...
                else
                {
                    ...
                    {
                        //@too el problema es que si quiero dar la posibildiad
                        // de que el código sea exception-free, esto no me vale,
                        //¿merece la pena?
                        try
                        {
                        ...
                        {
                            mel::text::error( "Error waiting for result. Err "
                                              "code = {}, Reason = {}",
                                              (int)e.getCode(), e.what() );
                        }
                        catch ( std::exception& e )
                        {
                            mel::text::error(
                                "Error waiting for result. Reason = {}",
                                e.what() );
                        }
                        catch ( string& msg )
                        {
                            mel::text::error(
                                "Error waiting for result. Msg = {}", msg );
                        }
                        catch ( ... )
                        {
                            mel::text::error(
                                "Error waiting for result. Unknown exception" );
                        }
                    }
                }
            },
    ...
    }
    {
        ...
        /*		 meditar sobre esto. Funciona bien pero tengo que
           dejarlo bien hilvanado: igual tengo que ponerle otro nombre, ya que
           viendo eso uno pensaría que sólo cambia el fut2, cuando en realidad
           cambia el data por lo que afecta a todos POSIBILDIADES:
                                  - llamarlo change		*/
        fut1.setValue( 6 );
        fut2.assign( fut1 );
    ...
    mel::text::set_level( mel::text::level::debug );
    {
        ...
        ;
        th1->post(
            [fut, &cont]( RUNNABLE_TASK_PARAMS ) mutable
            {
                // auto th = ThreadRunnable::getCurrentThreadRunnable();
                if ( cont == 1 )
                {
        ...
        /*		try
                        {
                                auto res =
           mel::core::waitForFutureThread<::mel::core::WaitErrorAsException>(fut);
                        ...
                        */
        auto res =
            mel::core::waitForFutureThread<::mel::core::WaitErrorNoException>(
                fut );
        if ( res.isValid() )
...
static int _testMicroThreadingMonoThread( tests::BaseTest* test )
{
    ...
    mel::text::info(
        "Process size {} ; GenericProcess size {}; MThreadAttributes {} ", s1,
        s2, s3 );

    {
        auto th1 = ThreadRunnable::create();
        mel::text::debug( "Request execution" );
        auto fut = th1->execute<int>(
            [th = th1.get()]
            {
                mel::text::debug( "Start function execution..." );
                if ( ::mel::tasking::Process::wait( 2000 ) !=
                     mel::tasking::Process::ESwitchResult::ESWITCH_OK )
                {
                    mel::text::error( "Task killed!!" );
                    Timer t;
                    auto t0 = t.getMilliseconds();
                    mel::text::debug(
                        "La espera siguiente no deberia tener efecto" );
                    ...
                    // now, try to execute "something" in same thread. Shoouldn
                    // do anything
                    //@todo ahora está impidiendo la espera correctamente. Lo
                    // que tengo que meditar es si debo impedir el execute como
                    // tal, ya que se está haciendo igual no tiene sentido
                    // porque
                    // el execute realmente se hace desde cualqueir sitio, no un
                    // mthread forzosamente
                    auto fut = th->execute<float>(
                        []
                        {
                            mel::text::error(
                                "This execution shouldn''t be done" );
                            return 1.5f;
                        } );
                    try
                    {
                        auto fr = ::mel::tasking::waitForFutureMThread( fut );
                        mel::text::info( "OK, execution was't done because "
                                         "process is killed" );
                    }
                    catch ( ... )
                    {
                        mel::text::error(
                            "Execution should have recieved a kill signal" );
                    }
                    /*
                    if ( !fr.isValid() && fr.error().error ==
                    ::mel::core::EWaitError::FUTURE_RECEIVED_KILL_SIGNAL)
                            text::info("OK, execution was't done because process
                    is killed"); else text::error("Execution should have
                    recieved a kill signal");
                    */
                }
    ...
    th1->post(
        [th2]( RUNNABLE_TASK_PARAMS )
        {
            auto& autokill = Runnable::killTrue;
            std::shared_ptr<Process> p1 = th2->post(
                [th2]( uint64_t t, Process* p )
                {
                    ...
                    if ( firstTime )
                    {
                        ...
                        try
                        {
                            auto fr = ::mel::tasking::waitForFutureMThread(
                                fut, 2000 );
                        ...
                        // if ( fr !=
                        // ::mel::core::FutureData_Base::EWaitResult::FUTURE_WAIT_OK)
                        // {
                        ...
                        // esto tengo que arreglarlo para que, si soy autokill,
                        // no vuelva a esperar si está en trying_tokill meditar
                        // sobre estos temas, no me convence demasiado la forma
    ...
    */
    ...
    /*
    preparar bien el test: quiero que los procesos actúa sobre algún objeto y
    tenga una salida precedible, por ejemplo:
...
int _testPerformanceLotTasks( tests::BaseTest* test )
{
    ...
    for ( int it = 0; it < nIterations; ++it )
    {
        for ( int j = 0; j < steps; ++j )
        {
            for ( int i = 0; i < th1->getMaxPoolSize(); ++i )
            {
                ++count;
                // th1->post<CustomProcessType,MyAllocator>(
                // [count](RUNNABLE_TASK_PARAMS)
                th1->post<false, MyAllocator>(
                    [count]( RUNNABLE_TASK_PARAMS )
                    { return ::mel::tasking::EGenericProcessResult::KILL; },
    ...
    t1 = sTimer.getMilliseconds();
    mel::text::info( "Time launching {} tasks with global new: {} msecs",
                     nTasks, t1 - t0 );
    ...
    t1 = sTimer.getMilliseconds();
    mel::text::info( "Time launching {} tasks with default allocator: {} msecs",
                     nTasks, t1 - t0 );
    // Thread::sleep(45000);
    return result;
}
...
{
    {
        ...
        constexpr int NUM_PRODUCERS = 7; // in github action increasing this
                                         // number leads to very high test time
        sCount = 0;
        mel::text::info( "Concurrent posts with normal sized buffer. Num "
                         "producers={}, num_posts={}",
                         NUM_PRODUCERS, NUM_POSTS );

        Runnable::RunnableCreationOptions opts;
        opts.schedulerOpts = ProcessScheduler::LockFreeOptions{};
        {
            auto consumer = ThreadRunnable::create(
                true,
                opts ); // en realidad en free lock el maxSize es el chunksize.
                        // Cuando l otenga como opciones ya lo haré conherente
            std::array<std::shared_ptr<ThreadRunnable>, NUM_PRODUCERS>
                producers;
            ...
            {
                producers[i]->post(
                    [consumer, NUM_POSTS]( uint64_t, Process* )
                    {
                        //	mel::tasking::Process::wait(50);//to wait for
                        // all producer posts done
                        for ( auto i = 0; i < NUM_POSTS; ++i )
                        {
                            //@todo así peta. Es algo de destruiccion del hilo,
                            // ya que esto hará que tarde mś que la espera a fin
                            // if (mel::tasking::Process::wait(1000) ==
                            // ::mel::tasking::Process::ESwitchResult::ESWITCH_OK)
                            {
                                consumer->fireAndForget( []() { ++sCount; }, 0,
                                                         Runnable::killFalse );
                            }
        ...
        {
            std::stringstream ss;
            ss << "Test concurrent posts KO!!. Some tasks were not executed. "
                  "Posts: "
               << numPosts << " Count: " << sCount.load();
    ...
    }
    {
        ...
        sCount = 0;
        mel::text::info( "Concurrent posts with very small sized buffer. Num "
                         "producers={}, num_posts={}",
                         NUM_PRODUCERS, NUM_POSTS );

        Runnable::RunnableCreationOptions opts;
        opts.schedulerOpts =
            ProcessScheduler::LockFreeOptions{ 10, 2 }; // provoca pete
        {
            auto consumer = ThreadRunnable::create(
                true,
                opts ); // en realidad en free lock el maxSize es el chunksize.
        ...
        [test]( RUNNABLE_TASK_PARAMS )
        {
            ...
            try
            {
                ...
                if ( _doWait( 200 ) == Process::ESwitchResult::ESWITCH_OK )
                {
                    ...
                    _throwExc( val );
                    test->setFailed(
                        "Task1: After throw exception. Shouldn't occur" );
                }
            }
            catch ( int v )
            {
                ...
                {
                    stringstream ss;
                    ss << "Task1: Invalid 'int' Captured exception. Thrown "
                       << val << " Catched " << v;
                    test->setFailed( ss.str() );
                }
                // igual, puedo ahcer algo de esto en el wait, restaurandolo a
                // la vuelta o similar..en este caso tengo que hacer buenas
                // pruebas con excepciones encadenadas aunque no tengo claro
                // como tendría que restaurarla.. mirar el uncaught_exceptions
                // el problema es que no puedo siempre "obligar" a que no se
                // haga u nswitch dentro de un catch, porque se podría llamar a
                // funciones desde aquí
                // auto except = std::current_exception();
                //_doWait(500);
            }
            catch ( MyException& exc )
            {
                if ( exc.code == val )
                {
                    mel::text::debug( "Task1: Captured exception ok. msg ={}",
                                      exc.msg );
                }
                else
                {
                    stringstream ss;
                    ss << "Task1: Invalid 'MyException' Captured exception. "
                          "Thrown "
                       << val << " Catched " << exc.code;
                    test->setFailed( ss.str() );
                    //					mel::text::error("Task1:onExecuteAlltests
                    // Captured exception Invalid,. Thrown {}, Catched
                    //{}",val,exc.code);
                }
                //	mel::tasking::Process::wait(500); //haciendo esto aquí,
                // peta al poco (Windows)
            }
    ...
    th1->post(
        [test]( RUNNABLE_TASK_PARAMS )
        {
            ...
            try
            {
                ...
                if ( _doWait( 300 ) == Process::ESwitchResult::ESWITCH_OK )
                {
                    ...
                    if ( rand() % 10 < 5 )
                        ...
                        _throwMyException( val );
                    test->setFailed(
                        "Task2: After throw exception. Shouldn't occur" );
                }
            }
            catch ( MyException& exc )
            {
                if ( exc.code == val )
                {
                    mel::text::debug( "Task2: Captured exception ok. msg ={}",
                                      exc.msg );
                }
                else
                {
                    stringstream ss;
                    ss << "Task2: Invalid 'MyException' Captured exception. "
                          "Thrown "
                       << val << " Catched " << exc.code;
                    test->setFailed( ss.str() );
                }
            ...
            catch ( int v )
            {
                ...
                {
                    stringstream ss;
                    ss << "Task2: Invalid Captured exception. Thrown " << val
                       << " Catched " << v;
                    test->setFailed( ss.str() );
                }
            }
    ...
    th1->post(
        [test]( RUNNABLE_TASK_PARAMS )
        {
            ...
            try
            {
                ...
                if ( _doWait( 500 ) == Process::ESwitchResult::ESWITCH_OK )
                {
                    ...
                    if ( rand() % 10 < 5 )
                        ...
                        _throwMyException( val );
                    test->setFailed(
                        "Task3: After throw exception. Shouldn't occur" );
                }
            }
            catch ( MyException& exc )
            {
                if ( exc.code == val )
                {
                    mel::text::debug( "Task3: Captured exception ok. msg ={}",
                                      exc.msg );
                }
                else
                {
                    stringstream ss;
                    ss << "Task3: Invalid 'MyException' Captured exception. "
                          "Thrown "
                       << val << " Catched " << exc.code;
                    test->setFailed( ss.str() );
                }
            ...
            catch ( int v )
            {
                ...
                {
                    stringstream ss;
                    ss << "Task3: Invalid Captured exception. Thrown " << val
                       << " Catched " << v;
                    test->setFailed( ss.str() );
                }
            }

...
void _sample2()
{
    ...
    th1->fireAndForget(
        [result]() mutable
        {
            ...
            try
            {
                auto res = waitForFutureMThread(
                    fut, 20 ); // will wait only for 20 msecs
    ...
    try
    {
        ...
        mel::text::info( "Result after waiting in currrent thread= {}",
                         res.value() );
    }
    catch ( mel::core::WaitException& e )
    {
        mel::text::error( "Error waiting in currrent thread. Reason = {}",
                          e.what() );
    }
    catch ( ... )
    {
...
void _sample3()
{
    ...
    auto th1 = ThreadRunnable::create(); // order is important here to respect
                                         // reverse order of destruction
    th1->fireAndForget(
        [&cs, &event]() mutable
        {
            ::mel::text::info(
                "Task1 Waiting before entering critical section" );
            ::mel::tasking::Process::wait( 1000 );
            Lock_mthread lck( cs );
            ::mel::text::info(
                "Task1 Entering critical section and set event" );
            event.set();
        },
        0, Runnable::killFalse );
    th1->fireAndForget(
...
void _sampleTasking1()
{
    ...
    constexpr unsigned int period =
        450; // how often, msecs, the tasks is executed
    auto& killPolicy = Runnable::killTrue;
    std::shared_ptr<mel::tasking::Process> task = th1->post(
        []( uint64_t msecs, Process* p )
        {
...
void _sampleTasking5()
{
    struct MyProcess : public Process
    {
        ...
        virtual bool onKill() override { return count == 10; }
...
void _sampleTasking6()
{
    ...
    th1->fireAndForget( [] { mel::text::info( "execute" ); }, 0,
                        Runnable::killTrue );
}
// custom factory
...
{
    // custom allocator which will use out CustomProcessFactory, not inheriting
    // from ProcessFactory, so best performance
    auto th1 = ThreadRunnable::create( true );
...
void _sampleCustomRunnable()
{
    class MyRunnable : public Runnable
    {
      public:
        ...
        void processTasks() { Runnable::processTasks(); }
    };
    MyRunnable r;
    r.post(
 ...
 * Disadvantages:
 *  - more memory footprint: because use internally en function, it has a fixed
 * size more than a simple function pointer
 *  - construction is more complex: you need to constructor the member passing a
 * callable adn a runnable Advantages:
 *  - No need to set runnable in header, which can be disturbing when using
 * macros
 *  - more understable: syntax more clean and (I think) more significant
 */
/*
//PRUEBAS
//TODO EL RUNNABLE NO PUEDE PASARSE POR CONSTRUCTOR, YA QUE CAMBIARÁ.
POSIBILIDADES PARA PASAR EL RUNNABLE: template <class TRet, class ...TArgs>
class SyncCallable
{
    public:
    problema gordo, necesitaría algo que haga referencia a un runnable, no
guardar el runnable en sí template <class F> SyncCallable(F&& callable,Runnable*
runnable):mRunnable(runnable),mCallable(std::forward<F>(callable)){}
        Future<TRet> operator()(TArgs&&...args)
        {
            //return mCallable(std::forward<TArgs>(args)...);
            return mRunnable->execute<TRet>(
std::bind(mCallable,std::forward<TArgs>(args)...)); \
        }
    private:
        Runnable* mRunnable;
...
*/
string f0_sync( int a, float b ) { return "hola"; }
...
void _sampleSyncMacros()
{
    ...
    {
        mel::text::info(
            "Calling static function f1 syncronized with a Runnable" );
        auto r = f1( 6 );
        try
        {
    ...
    }
    {
        MyClass obj;
        mel::text::info(
            "Calling static function f2 syncronized with a Runnable" );

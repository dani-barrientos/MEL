...
{
    std::cout << "<CTestMeasurement type=\"text/string\" name=\"" << name
              << "\">" << value << "</CTestMeasurement>\n";
}
void BaseTest::addMeasurement( string name, double value )
{
    std::cout << "<CTestMeasurement type=\"numeric/double\" name=\"" << name
              << "\">" << value << "</CTestMeasurement>\n";
}
// CDash documentation states that any numeric value is treated the same way, so
// really doesn't matter waht gos after "numeric/", but I introduce this
// overloads for possible future advances
void BaseTest::addMeasurement( string name, int value )
{
    std::cout << "<CTestMeasurement type=\"numeric/integer\" name=\"" << name
              << "\">" << value << "</CTestMeasurement>\n";
}
...
string BaseTest::getBuffer() const { return mTextBuffer.str(); }
...
bool BaseTest::checkOccurrences( string str, size_t n, const char* fileName,
                                 int lineNumber, LogLevel ll, bool useRegEx )
{
    size_t found;

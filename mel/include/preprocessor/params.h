...
*/
//TENGO QUE HACER PATRA�AS PARA QUE FUNCIONE EN VISUAL
#define _MEL_PREP_ARGS(_0, _1, ...) _1
#define TWO_ARGS(...) TWO_ARGS_IMPL( (__VA_ARGS__,1) )
#define TWO_ARGS_IMPL( args ) _MEL_PREP_ARGS args
...
*/
/**
* construcci�n de argumentos a partir de una lista tal que: (T1,T2,T3,...). Los Ti son tipos
* Construye una lista de la forma:(T1 _1, T2 _2, T3 _3,..)  (hasta 10 par�metros, puede ser ampliado f�cilmente
*/
	...
	#define CALL_PARAMS( ... ) CALL_PARAMS_IMPL( (MACRO_NUM_ARGS( __VA_ARGS__),__VA_ARGS__)  )
	#define CALL_PARAMS_IMPL( a )  CALL_PARAMS_N a 
	#define CALL_PARAMS_N(N,...)  CALL_PARAMS_(N, (__VA_ARGS__) )
	#define CALL_PARAMS_(N,args) CALL_PARAMS__(N, args)
	#define CALL_PARAMS__(N,args) CALL_PARAMS_## N args
	...
	#define CALL_PARAMS_10(...) CALL_PARAMS_9(),_10
	 ...
	 //para GCC. Estoy suponiendo que no hay m�s compiladores...

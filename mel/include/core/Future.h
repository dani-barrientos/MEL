    ...
    namespace core
    {
        ...
        enum class EWaitError : int
        {
            ...
            FUTURE_RECEIVED_KILL_SIGNAL =
                -1, //!< Wait for Future was interrupted
                    //!< because waiting Process was killed
            FUTURE_WAIT_TIMEOUT =
                -2, //!< Time out expired while waiting for Future
            ...
            : public std::variant<_private::NotAvailable, T, std::exception_ptr>
        {
            ...
            typedef std::variant<_private::NotAvailable, T, std::exception_ptr>
                Base;
          ...
          public:
            ...
            // wrapper for std::get.  Same rules as std::Get, so
            // bad_variant_access is thrown if not a valid value
            ReturnType value() { return std::get<T>( *this ); }
            CReturnType value() const { return std::get<T>( *this ); }
        ...
        {
            static constexpr size_t ValidIdx = 1;
            typedef std::variant<_private::NotAvailable, VoidType,
                                 std::exception_ptr>
                Base;
          ...
          public:
            ...
            // wrapper for optional::value().  Same rules as std::Get, so
            // bad_optional_access is thrown if not a valid value
            std::exception_ptr error() const
            {
                return std::get<std::exception_ptr>( *this );
            }
        ...
        namespace _private
        {
            ...
            class FutureData_Base
            {
              public:
                ...
                virtual ~FutureData_Base() // no me gusta nada que sea virtual.
                                           // Intentarlo sin ello
                {
                }
                inline bool getValid() const { return mState == VALID; }
                inline EFutureState getState() const { return mState; }
            ...
            class FutureData
                : public FutureData_Base,
                  public CallbackSubscriptor<
                      ::mel::core::CSMultithreadPolicy,
                      FutureValue<typename std::conditional<
            ...
            {
              public:
                typedef FutureValue<typename std::conditional<
                    std::is_lvalue_reference<T>::value,
                    std::reference_wrapper<
                        typename std::remove_reference<T>::type>,
                    T>::type>
                ...
                ~FutureData(){};
                /**
                 * constructor from value. It becomes valid. For casting
                 * purposes.
                 */
                template <class U>
                FutureData( U&& value ) : mValue( std::forward<U>( value ) )
                {
                ...
                template <class U> void setValue( U&& value )
                {
                ...
                template <class ET> void setError( ET&& ei )
                {
                    volatile auto protectMe = FutureData<T>::shared_from_this();
                    std::scoped_lock<std::recursive_mutex> lck(
                        FutureData_Base::mSC );
                    ...
                    {
                        mValue =
                            std::make_exception_ptr( std::forward<ET>( ei ) );
                        mState = INVALID;
                        Subscriptor::triggerCallbacks( mValue );
                    }
                };
                 ...
                 * @param f
                 * @return unsubscription id. -1 if not subscribed, which can
                 * occur when function is executed directly if future is valid
                 * and returns an unsubsscription
                 * @warning callback receiver shoudn't wait or do context switch
                 * and MUST NOT block
                 *
                 */
                template <class F> int subscribeCallback( F&& f )
                {
                    ...
                    {
                        result = Subscriptor::subscribeCallback(
                            std::function<::mel::core::ECallbackResult(
                                ValueType& )>(
                                [f = std::forward<F>( f )](
                                    ValueType& vt ) mutable
                                {
                                    f( vt );
                ...
                // because use of lambda, unsubscription can only be done by id
                // (returned by subscribeCallback)
            ...
            // specialization for void type. It's intented for functions
            // returning void but working in a different thread so user need to
            // know when it finish
            template <>
                  ...
                  public std::enable_shared_from_this<FutureData<void>>
            {
              ...
              public:
                ...
                {
                    volatile auto protectMe =
                        FutureData<void>::shared_from_this();
                    FutureData_Base::mSC.lock();
                    if ( mState == NOTAVAILABLE )
                    {
                        mValue.setValid();
            ...
            {
              public:
                FutureDataContainer( std::shared_ptr<FutureData_Base> ptr )
                    : mPtr( ptr )
                {
                }
                inline const std::shared_ptr<FutureData_Base>& getPtr() const
                {
                    return mPtr;
                }
                inline std::shared_ptr<FutureData_Base>& getPtr()
                {
                    return mPtr;
                }
                inline void
                setPtr( const std::shared_ptr<FutureData_Base>& ptr )
                {
                    mPtr = ptr;
                }
            ...
            class Future_Base
            {
              ...
              public:
                ...
                inline bool operator==( const Future_Base& f ) const noexcept
                {
                    return mData == f.mData;
                };
            };
        ...
        ///@endcond
        {
          protected:
            Future_Common()
            {
                mData = std::make_shared<_private::FutureDataContainer>(
                    std::make_shared<_private::FutureData<T>>() );
            };
          ...
          public:
             ...
             */
            inline const typename _private::FutureData<T>::ValueType&
            getValue() const
            {
                return static_cast<_private::FutureData<T>*>(
                           mData->getPtr().get() )
                    ->getValue();
            }
            /**
             ...
             * @return const ::mel::core::FutureValue
             */
            inline typename _private::FutureData<T>::ValueType& getValue()
            {
                return static_cast<_private::FutureData<T>*>(
                           mData->getPtr().get() )
                    ->getValue();
            ...
            void
            assign( const typename _private::FutureData<T>::ValueType& val )
            {
                getData().assign( val );
            }
            ...
            /**
             * @brief Makes this Future to point to the same value as the given
             * Future
             * @details This ways, both futures will share the same value/error.
             * If input Future is already set at that moment, callbacks are
             * triggered as usual. It's important to note that *all futures*
             * sharing same data will change
             */
            void assign( Future_Common<T> val )
            {
                auto ptr =
                    mData->getPtr(); // to avoid destruction before unlock
                std::scoped_lock<std::recursive_mutex, std::recursive_mutex>
                    lck( val.getData().getMutex(), getData().getMutex() );
                if ( val.getValue().isAvailable() )
                {
                    auto old = mData->getPtr();
                    setData( val.mData->getPtr() );
                    // if value is already available, tigger callbacks now. It's
                    // more efficient and avoid recursive calls
                    static_cast<_private::FutureData<T>*>( old.get() )
                        ->triggerCallbacks( getData().getValue() );
                }
                else
                {
                    val.getData().append(
                        std::move( getData() ) ); // append callbacks
                    setData( val.mData->getPtr() );
                }
            }
            /**
             * @brief Subscribe callback to be executed when future is ready
             * (valid or error)
             * @param[in] F Callable with signature `void( \ref
             * ::mel::core::FutureValue& )`
             * @warning Usually callback is executed in the context of the
             * thread doing setValue, but if Future is already ready callback
             * will be executed in the context of the thread doing subscription.
             * In general, for this and more reasons, callbacks responses should
             * be done buy launchin a task, so decoupliing totally the diferent
             * tasks
             * @return callback id
             */
            template <class F> int subscribeCallback( F&& f ) const
            {
                //@todo no me gusta un pijo este cast, pero necesito que el
                // subscribe
                // actúa como mutable
                return const_cast<Future_Common<T>*>( this )
                    ->getData()
                    .subscribeCallback( std::forward<F>( f ) );
            }
            /**
             * @brief Unsubscribe given callback
             * @details because use of lambda, unsubscription can only be done
             * by id (returned by subscribeCallback)
             */
            ...
            Future_Common( const Future_Common& f ) noexcept
                : _private::Future_Base( f )
            {
            }
            Future_Common( Future_Common&& f ) noexcept
                : _private::Future_Base( std::move( f ) )
            {
            }
          ...
          private:
            inline const _private::FutureData<T>& getData() const
            {
                return *static_cast<_private::FutureData<T>*>(
                    mData->getPtr().get() );
            }
            inline _private::FutureData<T>& getData()
            {
                return *static_cast<_private::FutureData<T>*>(
                    mData->getPtr().get() );
            }
            inline void
            setData( const std::shared_ptr<_private::FutureData_Base>& ptr )
            {
                mData->setPtr( ptr );
            }
        };
        /**
         * @brief Represents a value that *maybe* is not present at the current
         *moment.
         * @details This value will be generated by *someone* and will be
         *available at "some moment" The value is retrieved using various ways:
         *	- pooling in a loop checking for "getValid" and "error"
         *	- using wait functions, as ::mel::tasking::waitForFutureMThread
         *or
        ...
        template <typename T> class Future : public Future_Common<T>
        {
          public:
            ...
            Future( Future&& f ) noexcept
                : Future_Common<T>( std::move( f ) ){};
            Future( const T& val )
            {
                _private::Future_Base::mData =
                    std::make_shared<_private::FutureDataContainer>(
                        std::make_shared<_private::FutureData<T>>( val ) );
            }
            Future( T&& val )
            {
                _private::Future_Base::mData =
            ...
            template <class F> void setValue( F&& value )
            {
                static_cast<_private::FutureData<T>*>(
                    Future_Common<T>::mData->getPtr().get() )
                    ->setValue( std::forward<F>( value ) );
            }
            template <class F> void setError( F&& ei )
            {
                static_cast<_private::FutureData<T>*>(
                    Future_Common<T>::mData->getPtr().get() )
                    ->setError( std::forward<F>( ei ) );
            }
        };
        ///@cond HIDDEN_SYMBOLS
        template <typename T> class Future<T&> : public Future_Common<T&>
        {
          public:
            typedef typename _private::FutureData<T&>::ValueType ValueType;
            Future() noexcept {};
        ...
        // specialization for void
        template <> class Future<void> : public Future_Common<void>
        {
          public:
            ...
            {
                //				_private::Future_Base::mData =
                // std::make_shared<_private::FutureData<void>>(a);
                _private::Future_Base::mData =
                    std::make_shared<_private::FutureDataContainer>(
                        std::make_shared<_private::FutureData<void>>( a ) );
            };
            ...
            inline void setValue( void )
            {
                static_cast<_private::FutureData<void>*>(
                    Future_Common<void>::mData->getPtr().get() )
                    ->setValue();
            }
            template <class F> void setError( F&& ei )
            {
        ...
        /**
         * @brief Exception class generated by WaitResult when wait for future
         * gets an error
         *
         */
        class WaitException : public std::runtime_error
        {
          public:
            WaitException( EWaitError code, const std::string& msg )
                : mCode( code ), std::runtime_error( msg )
            {
            }
            WaitException( EWaitError code, std::string&& msg )
                : mCode( code ), std::runtime_error( std::move( msg ) )
            {
            }
        ...
        //! @brief Policy for treating error as exception in future wait
        //! functions
        struct WaitErrorAsException
        {
        };
        //! @brief Policy for not treating error as exception in future wait
        //! functions

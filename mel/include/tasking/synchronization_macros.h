...
#define CREATE_LINKER( TRet, function_name, args )                             \
    CREATE_LINKER_IMPL( MACRO_NUM_ARGS args, TRet, function_name, args )
#define CREATE_LINKER_IMPL( N, TRet, function_name, args )                     \
    CREATE_LINKER_IMPL_( N, TRet, function_name, args )
#define CREATE_LINKER_IMPL_( N, TRet, function_name, args )                    \
    CREATE_LINKER_N( N, TRet, function_name, args )
//#define CREATE_LINKER_N( N,TRet,function_name,args) CREATE_LINKER_ ## N(
// TRet,function_name, args  )
#define CREATE_LINKER_N( N, TRet, function_name, args )                        \
    CREATE_LINKER_N_( N, TRet, function_name, args )
#define CREATE_LINKER_N_( N, TRet, function_name, args )                       \
    CREATE_LINKER_##N( TRet, function_name, args )
...
#define CREATE_LINKER_2( TRet, function_name, args )                           \
    linkFunctor<TRet, TYPELIST(), GET_ARG_1 args, GET_ARG_2 args>(             \
        function_name, _1, _2 )
#define CREATE_LINKER_3( TRet, function_name, args )                           \
    linkFunctor<TRet, TYPELIST(), GET_ARG_1 args, GET_ARG_2 args,              \
                GET_ARG_3 args>( function_name, _1, _2, _3 )
#define CREATE_LINKER_4( TRet, function_name, args )                           \
    linkFunctor<TRet, TYPELIST(), GET_ARG_1 args, GET_ARG_2 args,              \
                GET_ARG_3 args, GET_ARG_4 args>( function_name, _1, _2, _3,    \
                                                 _4 )
#define CREATE_LINKER_5( TRet, function_name, args )                           \
    linkFunctor<TRet, TYPELIST(), GET_ARG_1 args, GET_ARG_2 args,              \
                GET_ARG_3 args, GET_ARG_4 args, GET_ARG_5 args>(               \
        function_name, _1, _2, _3, _4, _5 )

#define CREATE_LINKER_6( TRet, function_name, args )                           \
    linkFunctor<TRet, TYPELIST(), GET_ARG_1 args, GET_ARG_2 args,              \
                GET_ARG_3 args, GET_ARG_4 args, GET_ARG_5 args,                \
                GET_ARG_6 args>( function_name, _1, _2, _3, _4, _5, _6 )
#define CREATE_LINKER_7( TRet, function_name, args )                           \
    linkFunctor<TRet, TYPELIST(), GET_ARG_1 args, GET_ARG_2 args,              \
                GET_ARG_3 args, GET_ARG_4 args, GET_ARG_5 args,                \
                GET_ARG_6 args, GET_ARG_7 args>( function_name, _1, _2, _3,    \
                                                 _4, _5, _6, _7 )

        /*
        old mpl functions/classes haven't any noexcept considerations (not
        existed at all in C++98), so changed in favor of a labmda based
        mechanism
                #define SYNCHRONIZED_STATIC( TRet,
        function_name,args,runnable,qualifiers) \
                        static TRet function_name##_sync args qualifiers; \
                        static Future<TRet> function_name( MAKE_PARAMS args  )
        qualifiers{ \ return runnable->execute<TRet>(  CREATE_LINKER(
        TRet,function_name##_sync,args ) );}
                #define SYNCHRONIZED_METHOD( TRet,
        function_name,args,runnable,qualifiers) \
                        TRet function_name##_sync args qualifiers; \
                        Future<TRet> function_name( MAKE_PARAMS args  )
        qualifiers{ \ return runnable->execute<TRet>(  CREATE_LINKER(
        TRet,makeMemberEncapsulate(
        &std::remove_reference_t<decltype(*this)>::function_name##_sync,this),args
        ) );}
        */
        ///@endcond
/**
* @brief Declare a static function synchronized with a given runnable.
* @details The code need to implement that *no sync* funcion (so, a normal
funcion) which its call is managed by this macro. For example, if
* we declare a synchronized function as \verbatim SYNCHRONIZED_STATIC(
f1,string,(int),noexcept,sRunnable ) \endverbatim, we have to implement the
neccesary code for `f1_sync`, as:
* \code
*
...
* @param[in] args parameter types (without parameter names), in the form
(T1,T2,...) [0..10] number of parameters
...
* @details The code need to implement that *no sync* funcion (so, a normal
funcion) which its call is managed by this macro. For example, if
* we declare a synchronized function as \verbatim SYNCHRONIZED_METHOD(
f1,string,(int,float),,sRunnable ) \endverbatim, we have to implement the
neccesary code for `f1_sync`, as:
* \code
*

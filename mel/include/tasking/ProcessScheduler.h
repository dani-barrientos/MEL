...
namespace mel
{
    namespace tasking
    {
         ...
         * @version 1.0
         * @remarks as is explained in Process, temporization is used with a
         * Timer but getting only the 32 bit low part of the 64 bits time
         * returned, for efficiency reasons.
         */
        class MEL_API ProcessScheduler
        {
          ...
          public:
            ...
            struct LockFreeOptions
            {
                ...
                size_t maxChunks = 4; //!< maximun number of blocks to allocate
                                      //!< when growing, before reset
            };
             ...
             * @brief Constructor
             *
             * @param initialPoolSize  initial size of pool tasks, that will
             * grow if neccesary. if > maxNewTasks, is assigned same value (only
             * for lock-free mode)
             * @param maxNewTasks maximum task to grow the pool. When this size
             * is overcame, the buffer is reseted.(only for lock-free mode)
             */
            ProcessScheduler( SchedulerOptions opts );
            ~ProcessScheduler( void );

            /**
             * insert new process to be executed. Execution order doesn't
             * necessarily respect the post order
             *
             * @param process insert new process in scheduler
             * @param starttime msecs to wait to start it
             * @warning 	if process is already inserted behaviour is
             * unpredictable
             */
            void insertProcess( std::shared_ptr<Process> process,
                                unsigned int startTime = 0 );
            /**
             * @brief Insert new process in scheduler without locking
             * processlist
             * @details Use this function very carefully, when you know that no
             * other thread is inserting processes or executing it
             *
             * @param process
             * @param startTime
             */
             ...
             * @todo no me gusta, hay que generalizarlo mejor. Como obtener los
             * procesos que cunplen algo y actuar sobre ellos
             */
            ...
            /**
            * send processes kill signal. It doesn�t kill inmediately, it's done
            at execution cycle begin, so
            * you need to continue processing until no more process are pending
            (getProcessCount() == 0)
            * @param[in] deferred If true, then kill will be posted as a new
            process. If False, it's done inmediately.
            * First type is when done into Scheduler context and second form is
            when done from another thread
            @todo no me gusta
            */
             ...
             * @return true if the task has already been executed. false
             * otherwise
             * @warning  this function is not thread-safe. If you need it you
             *  must block executeprocess() and checkFor() with a critical
             * section
             */
          ...
          private:
            ...
            /**
             * internal method intended to be used by Process to notify was
             * awakened
             * @todo maybe this will be deprecated in favor of another mechanism
             * (CallbackSubscriptor, etc)
             */

...
namespace mel
{
     ...
     * @details Based on the concept of *microthread*, which is represented by
     * class Process. A microthread alows to have cooperative multitasking, so
     * that a single thread can execute thousands concurrent (in a *cooperative*
     * sense ) tasks. Think microthread as a very light fiber
     */
    namespace tasking
    {
         ...
         * @details Once the process is inserted in a \ref
         * ::mel::tasking::ProcessScheduler (usually done through a Runnable via
         * ::mel::tasking::Runnable::post or ::mel::tasking::Runnable::execute)
         * the update() (private) member function will be called. because this
         * class is **abstract**, a concrete %Process must implement the
         * function onUpdate function to code its custom behaviour. The task
         * will be executed as time intervals, given by the period (\ref
         * getPeriod) until is killed (see \ref kill)
         * @remarks  Tasks are executed according to a \ref ::mel::core::Timer
         * "Timer", which uses uint64_t type to express msecs, but for eficiency
         * reasons using a 64 bits type on 32 bits machines will be very
         * "agressive", so we use unsigned int as time for tasks so getting the
         * low part of the original 64 bit time and hoping it will be enough...
         * @remarks Switching a Process inside a try/catch context doesn't fit
         * very well with exception handling for this context. This issue is
         * being addressed... If this fact is carefully considered when doing
         * "switchs" it will not be a problem, but in Windows (maybe other
         * Operating Systems?) there is a exploitation prevent system (called
         * SEHOP, see
         ...
         * )that will make the app crash because Windows interpret it as a hack
         * process. This option is disabled in worksations but enabled for
         * Windows Server- To disable it, go to
        ...
        ///@endcond
        {
          ...
          public:
            enum EProcessState : uint8_t
            {
                ...
                PREPARED_TO_DIE, //!< it's going to die, but process manager
                                 //!< doesn't discard it yet
                TRYING_TO_KILL,  //!< sending kill signal but no accepted yet
                KILLING_WAITING_FOR_SCHEDULED, //!< switched and no shceduled
                                               //!< yet
            ...
            enum class ESwitchResult
            {
                ...
                ESWITCH_WAKEUP, //!< return from context switch was because a
                                //!< wakeup
            ...
            /**
             * @brief constructor
             * @param capacity Initial stack capacity,in bytes. A value of 0
             * means no stack precreated. In any case, stack grows as needed
             */
            Process( unsigned short capacity = 0 );
            virtual ~Process( void );
            /**
             * @brief Sets ProcessScheduler which holds this process. Each
             * process can only be scheduled by one ProcessScheduler
             */
             ...
             * @details Internally, kill calls virtual function Process::onKill,
             * which returns true if kill can be acomplished or not. In case of
             * true, then process is put in PREPARED_TO_DIE state, so the
             * scheduler can remove it. In case of false, process is put in a
             * TRYING_TO_KILL state so scheduler will continue to trying to kill
             * it in next iterations. You can know if a process is die checking
             * getDead
             * @remarks if process is not already inited (state == PREPARED)
             * then onKill is not called @todo CAMBIAR ESTO?
             *
             * @param[in] force if true, then killing is without regarding
             * previous explained process, so Process go inmediately to
             * PREPARED_TO_DIE
             *
             * @remarks not multithread safe, kill should be done in the owner
             * thread context (se abordar� en el futuro)
             */
            ...
            /**
             * @return time when task was executed in last iteration, when
             * onUpdate is called:
             * @note context switches or waits inside code doesn't modify update
             * time.
             */
            ...
            /**
             * @brief Evict current process and will continue executing from the
             * same point the next time is scheduled
             * @details this function is a static one because, in the same way
             * threads work, is the *currently executing process* which can be
             * evicted in any point of the execution call stack.
             * @param continueInmediately if true, the process will continue
             * executing in the next shceduler cycle, so without respecting the
             * period.
             */
            static ESwitchResult
            switchProcess( bool continueInmediately ) OPTIMIZE_FLAGS;
            /**
             * @brief Sleep current process and execute given callable. To
             * reactivate to you must use \ref wakeUp
             * @param[in] postSleep functor (signature void(void) ) to execute
             * just in the moment when Process go to sleep
             ...
             * @param[in] postWait functor (signature <void,void>) to execute
             * just in the moment when Process go to sleep
            ...
            /**
             * @brief pause **current process**. To reactivate you must use
             * wakeUp
             * @return true if process received kill signal
             * @see switchProcess for comments
             */
            ...
            /**
             * @brief wakeup an asleep process or an evicted process (that
             * process having called swtich or wait)
             */
            void wakeUp();
          ...
          private:
            ...
            ProcessScheduler*
                mOwnerProcessScheduler; //!< scheduler in which is inserted
            /**
          ...
          protected:
            /**
             * behaviour function. The main function of a process. MUST be
             * implemented in childs. It's called automatically when elapsed
             * time >= mPeriod
             *
            ...
            /**
             * called when process is going to be got out of scheduler and was
             * inited (not in PREPARED state) can be overridden by children

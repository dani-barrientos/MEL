...
namespace mel
{
    ...
    {
        /**
         * @brief A Process constructed from a functor with signature
         * *EGenericProcessResult(uint64_t,Process*)*
         * @details This process it's executed until given functor returns \ref
         * EGenericProcessResult::KILL.
         */
        class MEL_API GenericProcess : public Process
        {
          public:
            typedef std::function<EGenericProcessResult( uint64_t, Process* )>
                GenericCallback;
          ...
          public:
             ...
             * @param[in] functor A functor with signature
             * EGenericProcessResult(uint64_t,Process*)
             */
            template <class F> void setProcessCallback( F&& functor );
            inline const GenericProcess::GenericCallback&
            getProcessCallback() const;
        ...
        template <class F>
        void GenericProcess::setProcessCallback( F&& functor )
        {
            mProcessCallback = std::forward<F>( functor );
        }
        const GenericProcess::GenericCallback&
        GenericProcess::getProcessCallback() const
        {

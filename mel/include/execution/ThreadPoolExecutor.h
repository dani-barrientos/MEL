...
namespace mel
{
    namespace execution
    {
        ...
        template <> class Executor<ThreadPool>
        {
          public:
            ...
            Executor( Executor&& ex )
                : mPool( std::move( ex.mPool ) ), mOpts( ex.mOpts )
            {
            }
            Executor( const Executor& ex )
                : mPool( ex.mPool ), mOpts( ex.mOpts )
            {
            }
            ...
            void launch( F&& f, TArg&& arg,
                         ExFuture<ThreadPool, TRet> output ) const noexcept
            {
                if ( !mPool.expired() )
                {
                ...
                else
                {
                    output.setError( new std::runtime_error(
                        "ThreadPoolExecutor::launch. Pool has expired!!!" ) );
                }
            }
                         ...
                         ExFuture<ThreadPool, TRet> output ) const noexcept
            {
                if ( !mPool.expired() )
                {
                    ...
                    opts.schedPolicy = ThreadPool::SchedulingPolicy::SP_BESTFIT;
                    auto th = mPool.lock()->selectThread( opts );
                    th->execute<TRet>( std::forward<F>( f ),
                                       static_cast<Future<TRet>>( output ),
                                       mOpts.autoKill ? Runnable::killTrue
                                                      : Runnable::killFalse );
                }
            ...
            parallel( ExFuture<ThreadPool, TArg> fut, std::exception_ptr& excpt,
                      FTypes&&... functions );
            template <class ReturnTuple, class TArg, class... FTypes>
            ::mel::parallelism::Barrier
            parallel_convert( ExFuture<ThreadPool, TArg> fut,
                              std::exception_ptr& except, ReturnTuple& result,
                              FTypes&&... functions );
            ///@}
          private:
        ...
        template <class I, class F>
        ::mel::parallelism::Barrier
        Executor<ThreadPool>::loop( I&& begin, I&& end, F&& functor,
                                    int increment )
        {
            static_assert( std::is_invocable<F, I>::value,
                           "ThreadPoolExecutor::loop bad functor signature" );
            ThreadPool::ExecutionOpts exopts;
            exopts.useCallingThread = false;
            exopts.groupTasks = !getOpts().independentTasks;
            return ::mel::parallelism::_for(
                getPool().lock().get(), exopts, std::forward<I>( begin ),
                std::forward<I>( end ), std::forward<F>( functor ), increment );
        }
        ///@cond HIDDEN_SYMBOLS
        namespace _private
        {
            ...
            {
                typedef typename mel::execution::ExFuture<ThreadPool, T>
                    FutType;
              ...
              public:
                ...
                {
                    return mFut.getValue().isAvailable();
                }
        ...
        template <class TArg, class... FTypes>
        ::mel::parallelism::Barrier
        Executor<ThreadPool>::parallel( ExFuture<ThreadPool, TArg> fut,
                                        ...
                                        FTypes&&... functions )
        {
            ...
            if constexpr ( std::is_same<TArg, void>::value )
                return getPool().lock()->execute(
                    exopts, except, std::forward<FTypes>( functions )... );
            else
                return getPool().lock()->execute(
                    exopts, _private::ValueWrapper<TArg>( fut ), except,
                    std::forward<FTypes>( functions )... );
        }
        template <class ReturnTuple, class TArg, class... FTypes>
        ::mel::parallelism::Barrier Executor<ThreadPool>::parallel_convert(
            ExFuture<ThreadPool, TArg> fut, std::exception_ptr& except,
            ReturnTuple& result, FTypes&&... functions )
        {
            ...
            exopts.groupTasks = !getOpts().independentTasks;
            if constexpr ( std::is_same<TArg, void>::value )
                return getPool().lock()->executeWithResult(
                    exopts, result, except,
                    std::forward<FTypes>( functions )... );
            else
                return getPool().lock()->executeWithResult(
                    exopts, result, _private::ValueWrapper<TArg>( fut ), except,
                    std::forward<FTypes>( functions )... );
        }
        /**
         ...
         */
        template <>
        struct ExecutorTraits<Executor<ThreadPool>> : ExecutorTraits<void>
        {
            enum
            {

...
namespace mel
{
    ...
    {      
        namespace flow
        {
             ...
             */
            template <class TArg,class ExecutorAgent,class ...Flows> FlowsResult< typename ::mel::execution::_private::GetReturn<ExFuture<ExecutorAgent,TArg>,Flows...>::type>
            ...
            namespace _private
            {
                ...
                template <class ...Flows> struct ApplyLaunch
                {
                    ...
                    {                  
                        return launch(inputFut,std::forward<Flows>(std::get<Flows>(mFuncs))...);
                    }
                };    
            }
            
            ///@brief version for use with operator |
            template <class ...Flows> _private::ApplyLaunch<Flows...> launch(Flows&&... flows)
            {

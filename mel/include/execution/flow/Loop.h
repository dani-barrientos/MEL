...
namespace mel
{
    ...
    {      
        namespace flow
        {
             ...
             * @param getIteratorsFunc Callable returning an std::array<I,2> with begin and end iterators to loop in
            ...
            template <class TArg,class ExecutorAgent,class Flow,class I> ExFuture<ExecutorAgent,void>
                loop(ExFuture<ExecutorAgent,TArg> source, I getIteratorsFunc, Flow flow,int increment = 1)
            {
                ...
                int length;
                //@todo pending to pass input result to this function. The problem I see is about error management: is there is an error, this function can't
                //be called, so the flows won't be able to catch the error
                auto iterators = getIteratorsFunc();
                using IteratorType = decltype(iterators[0]);
                constexpr bool isArithIterator = mel::mpl::TypeTraits<typename std::decay<IteratorType>::type>::isArith;
            ...
            {                                          
                template <class I,class F> struct ApplyLoop
                {
                    template <class U,class T> 
                        ApplyLoop(U&& its, T&& f,int inc):mGetIts(std::forward<U>(its)),mFlow(std::forward<T>(f)),increment(inc){}
                    I mGetIts;
                    F mFlow;                    
                    int increment;
                    template <class TArg,class ExecutorAgent> auto operator()(ExFuture<ExecutorAgent,TArg> inputFut)
                    {                  
                        return flow::loop(inputFut,std::forward<I>(mGetIts),std::forward<F>(mFlow),increment);
                    }
                };    
            }
            
            ///@brief version for use with operator |
            template <class I,class F> flow::_private::ApplyLoop<I,F> loop(I&& getItFunc, F&& flow, int increment = 1)
            {
                return _private::ApplyLoop<I,F>(std::forward<I>(getItFunc),std::forward<F>(flow),increment);
            }
        }
    }
}
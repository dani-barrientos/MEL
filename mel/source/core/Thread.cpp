...
namespace mel
{
    namespace core
    {
        ...
        unsigned int getNumProcessors()
        {
            ...
            BOOL ok = GetProcessAffinityMask( hProcess, &processAffinity,
                                              &systemAffinity );
...
#define HANDLETYPE pid_t
        ...
        static bool _setAffinity( uint64_t affinity, HANDLETYPE h )
        {
            ...
            result = oldAff != 0;
#elif defined( MEL_MACOSX ) || defined( MEL_IOS )
            //@todo macos X (and assume same for iOS) doesn't allow to set the
            // core for each thread. instead, it uses "groups" indetifying which
            // threads belong to same gruop and then share L2 cache. So, this is
            // not the concept intended with current function, and so leave ti
            // unimplemented
            /*
            ...
            auto err = thread_policy_set(mach_thread1, THREAD_AFFINITY_POLICY,
            (thread_policy_t)&policyData, 1); switch(err)
            {
                    case KERN_SUCCESS:
                            result = true;
                            break;
                    case KERN_NOT_SUPPORTED:
                            mel::text::error("Error setting thread affinity.
            Operation not supported"); break; default: mel::text::errorf("Error
            setting thread affinity: %d",err);
            }
            */
            return true;
#elif defined( MEL_LINUX ) || defined( MEL_ANDROID )
            ...
            // pthread_setaffinity_np(mHandle, sizeof(cpuset), &cpuset); ->no
            // existe en android int ok = syscall(__NR_sched_setaffinity,
            // mHandle, sizeof(affinity), &affinity);
...
#elif defined( MEL_MACOSX ) || defined( MEL_IOS )
            return _setAffinity(
                affinity, 0 ); // i don't know how to get current thread id on
                               // macos, but it doesn't matter because affinity
                               // in this sense can't be modified
#else
        ...
        /*
                ...
                #endif
                #if defined(MEL_LINUX) || defined(MEL_ANDROID) ||
           defined(MEL_IOS) || defined (MEL_MACOSX) void* _threadProc(void*
           param) { Thread* t=(Thread*)param; assert(t && "NULL Thread!"); if (
           t->mAffinity != 0 ) t->setAffinity(t->mAffinity);

                // esto tengo que estudiarlo a ver si realmente es necesario. no
           me fío
                // #if defined(MEL_MACOSX) || defined(MEL_IOS)
                // 		//Create the auto-release pool so anyone can
           call Objective-C code safely
...
#define __PLATFORM_POLICY SCHED_OTHER
...
#if defined( MEL_MACOSX ) || defined( MEL_LINUX ) || defined( MEL_IOS ) ||     \
    defined( MEL_ANDROID )
        ...
        bool Thread::join( unsigned int millis )
        {
            if ( mJoinResult == EJoinResult::JOINED_NONE && mThread.joinable() )
            {
                ...
                catch ( std::system_error& e )
                {
                    ...
                    text::error( "Error joining thread. Code = 0x{:x}. Msg={}",
                                 ec.value(), ec.message() );
            ...
            {
                    text::warn("Thread join done from same thread. deadLock!!.
    Ignoring join");
            }
            else
            {
                    ...
                    switch(status)
                    {
                            ...
                            case WAIT_FAILED:
                            {
                                    ...
                                    text::error("Error joining thread. Err =
    0x{:x}",err);
                            }
                                    break;
                                    //@todo log en deadlock
                            case WAIT_TIMEOUT:
                                    text::error("Error joining thread.Time out
    exceeded"); default: break;
            ...
            if ( err != 0)
            {
                    ...
                    {
                            case EDEADLK:
                                    text::error("Error joining thread,
    deadloack. Maybe thread is destroyed from this thread context"); break; case
    EINVAL: text::error("Error joining thread. Thread not joinable"); break;
                            case ESRCH:
                                    text::error("Error joining thread. Thread
    doesn't exist"); break;
                    }
            }else
                    mJoinResult = EJoinResult::JOINED_OK;
    ...
    #elif defined(MEL_LINUX) || defined(MEL_ANDROID) || defined(MEL_IOS) ||
    defined (MEL_MACOSX) if (yp==YP_ANY_THREAD_ANY_PROCESSOR) {
                    Thread::sleep(0);
            }
            else {
                    sched_yield();
    ...
    */
        ...
        void Thread::setPriority( EThreadPriority tp )
        {
            ...
            mPriority = tp;
#ifdef _WINDOWS
            BOOL r = SetThreadPriority( mThread.native_handle(),
                                        priority2WindowsAPI( tp ) );
            if ( !r )
                mel::text::error(
                    "Error setting thread priority. Error code = 0x{:x}",
                    GetLastError() );
#else
            sched_param sp;
            sp.sched_priority =
                priority2pthread( mPriority, mPriorityMin, mPriorityMax );

            int ret = pthread_setschedparam( mThread.native_handle(),
                                             __PLATFORM_POLICY, &sp );
            if ( ret )
                mel::text::warn(
                    "Error setting thread priority. Error code = 0x{:x}", ret );
#endif
        }
        /*	#ifdef _WINDOWS
                ...
                void Thread::setPriority(ThreadPriority tp) {
                        @todo resolver, seguramente tenga que seguir haciendolo
           de forma nativa if (mPriority==tp) return; mPriority=tp;

                        if (mHandle) {
                                sched_param sp;
                                sp.sched_priority =
           priority2pthread(mPriority,mPriorityMin,mPriorityMax); #ifdef
           MEL_ANDROID #define __PLATFORM_POLICY SCHED_OTHER #else
                                //SCHED_RR vs SCHED_FIFO?
                                #define __PLATFORM_POLICY SCHED_RR

                #endif
                                int ret = pthread_setschedparam(mHandle,
           __PLATFORM_POLICY, &sp); #pragma unused(ret) assert(!ret);
        ...
        */
        ...
        bool Thread::setAffinity( uint64_t affinity )
        {
            ...
            //pthread_setaffinity_np(mHandle, sizeof(cpuset), &cpuset); ->no
    existe en android
            //int ok = syscall(__NR_sched_setaffinity, mHandle,
    sizeof(affinity), &affinity); if (mThHandle != 0)
            {
                    int ok = sched_setaffinity(mThHandle, sizeof(cpuset),
    &cpuset); result = (ok == 0); if (!result)
                    {
                            int err = errno;
                            //text::error("Error setting thread affinity. {}",
    err);
                    }
            }
            else
    ...
    mel::core::Event::EWaitCode
    mel::core::waitForBarrierThread( const ::mel::parallelism::Barrier& b,
                                     unsigned int msecs )
    {
        ...
        struct _Receiver
        {
            ...
            Event::EWaitCode wait( const ::mel::parallelism::Barrier& barrier,
                                   unsigned int msecs )
            {

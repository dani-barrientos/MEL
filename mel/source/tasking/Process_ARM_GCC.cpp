...
volatile void Process::checkMicrothread( uint64_t msegs )
{
    if ( !mSleeped )
    {
        ...
        asm volatile( "str sp,[r2,%[v]]" ::[v] "J"(
            mIniSPOFF ) ); // mInisP = sp TODO ESTE YA NO ME VALE??
        // mter en LR la etiqueta exitFunction para retornar en el switch
        asm volatile( "ldr r3,=exitFunction" );
        asm volatile( "str r3,[r2,%[v]]" ::[v] "J"(
            mLROFF ) ); // a donde retornar tras el switch
        asm volatile(
            "ldr r3, [r2,%[v]]" ::[v] "J"( mSwitchedOFF ) ); // R2<--mSwitched
        asm volatile( "cmp r3,#0\n"
                      " beq continueExecute" );
        asm volatile( "mov r3,#0" );
        asm volatile(
            "str r3,[r2,%[v]]" ::[v] "J"( mSwitchedOFF ) ); // mSwitched = false
        // restore stack
        //	asm volatile( "add sp,sp,#4" ); //POR QUE???
        asm volatile(
            "ldr r1,[r2,%[v]]" ::[v] "J"( mStackEndOFF ) ); // r1 <-- stackend
        asm volatile(
            "ldr r3,[r2,%[v]]" ::[v] "J"( mStackOFF ) ); // r3 <-- inicio pila
        ...
        asm volatile(
            "continueExecute:" ); // si mSwited== 0 --> sigue ejecutando
...
volatile void fakeFunction()
{
    ...
    asm volatile(
        "str r1, [r0,%[v]]" ::[v] "J"( mSwitchedOFF ) ); // R2<--mSwitched
    ...
    // asm volatile( "ldr sp,[r0,#12]" ); SE SUPONE QUE EL BUCLE ANTERIOR YA LO
    // HACE asm volatile( "add sp,#20" ); //a pelo, tengo que buscar el
    // est�ndard retornar al punto de llamada

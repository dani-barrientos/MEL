...
volatile void Process::checkMicrothread( uint64_t msegs )
{
    ...
    asm volatile( "ldr r4,%[v]" ::[v] "m"( thisAux )
                  : "r4", "r0",
                    "r1" ); // tengo que enga�arle aqui para que tenga en cuenta
                            // que se modifican, ya que se hace cuando se
                            // produce un cambio de contexto

    asm volatile( "str sp, [r4,%[v]]" ::[v] "i"( mIniSPOFF ) );

    asm volatile( "add r4,r4,%[v]" ::[v] "i"( mRegistersOFF ) : "r4" );
    // asm volatile( "stmia r4!,{r0-r12}");  //->en version iOS, est� mal
    // ajustado el r4, debe apuntar a uno m�s bloque funcional android
    asm volatile( "stmia r4,{r0-r12}" );
    // ajuste de posicion
    asm volatile( "add r4,r4,%[v]" ::[v] "i"( mCoRegistersOFF - mRegistersOFF )
                  : "r4" );
    // fin bloque android

    asm volatile( "vstmia r4,{d0-d15}" );
    asm volatile( "sub r4,r4,%[v]" ::[v] "i"( mCoRegistersOFF )
                  : "r4" ); // restauramos posicion r4 al principio
    asm volatile( "str r4,[r4,%[v]]" ::[v] "i"(
        mRegistersOFF +
        16 ) ); // guar R4 en su posicion regitros + 4*4 --> MEJORAR

    asm volatile(
        "mov r5,#0\n"
        "add r5,pc,#64" ::
            : "r5" ); // TODO no se como obtener la direccion de un label!!
    ...
    // asm volatile( "ldmia r4!,{r5-r12}"); version iOS, no entiendo como
    // funciona puesto que r4 tiene que quedar mal colocado (4 bytes menos), es
    // decir, apunta a ultimo elemento
    asm volatile( "ldmia r4,{r5-r12}" );
    // ajustar posicion
    asm volatile(
        "add r4,r4,%[v]" ::[v] "i"( mCoRegistersOFF - ( mRegistersOFF + 20 ) )
        : "r4" );
    asm volatile( "vldmia r4,{d0-d15};" );
}

volatile void fakeFunction() __attribute__( ( noinline ) )
...
volatile void fakeFunction()
{
    ...
    asm volatile(
        "ldr r4,[r0,%[v]]" ::[v] "i"( mRegistersOFF + 16 ) ); // 16 = 4*4
...
volatile void Process::checkMicrothread( uint64_t msegs )
{
    ...
    asm volatile(
        "ldr x10,%[v]" ::[v] "m"( thisAux )
        : "x10", "x0",
          "x1" ); // tengo que enga�arle aqui para que tenga en cuenta que se
                  // modifican, ya que se hace cuando se produce un cambio de
                  // contexto (y desde su punto de vista no hay nada de eso, por
                  // lo que el compilador podr�a no salvagaurdar estos y por
                  // tanto error al usarlos al retornar) store return label
                  // address in local x11. Can be calcualted directly as a
                  // constant because address calculation if mor complicated,
                  // involving adr instruction
    ...
    // recover stack
    asm volatile( "ldr x11,[x10,%[v]]" ::[v] "i"( mStackEndOFF )
                  : "x11" ); // is set by _resizeStack
...
volatile void fakeFunction()
{
    ...
    asm volatile(
        "ldr x20,[x0,%[v]]" ::[v] "i"( mIniSPOFF ) ); // SP at function begin
    ...
    asm volatile( "mov x0,x19" ); // restore x0 (=this)
                                  ...
                                  // inisp is in x20
    asm volatile(
        "ldr x2,[x0,%[v]]" ::[v] "i"( mStackOFF ) ); // local stack base
    ...
    asm volatile(
        "br x2 " ); // no estoy seguro si RET x2 me valdr�a o ser�a el correcto
}
void Process::_switchProcess()
{
    auto p = ProcessScheduler::getCurrentProcess().get();

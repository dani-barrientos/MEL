...
void* ::mel::tasking::_private::RunnableTask::operator new( size_t s,
                                                            Runnable* owner )
{
...
void ::mel::tasking::_private::RunnableTask::operator delete(
    void* ptr ) noexcept
{
    ::operator delete( ptr );
    /*	Runnable* ownerRunnable;
            RTMemBlock* block = (RTMemBlock*)((char*)ptr - offsetof( RTMemBlock,
       task)); ownerRunnable = block->owner->owner;
...
void ::mel::tasking::_private::RunnableTask::operator delete(
    void* ptr, Runnable* ) noexcept
{
    RTMemBlock* mBlock =
        (RTMemBlock*)( (char*)ptr - offsetof( RTMemBlock, task ) );
...
RTMemPool* Runnable::_addNewPool()
{
    ...
    auxPool.owner = this;
    auxPool.pool =
        (RTMemBlock*)malloc( sizeof( RTMemBlock ) * mOpts.maxPoolSize );
    auxPool.count = 0;
...
Runnable::RunnableInfo* Runnable::_getCurrentRunnableInfo()
{
    ...
    // if (gCurrentRunnableKeyCreated) //not multithread-safe but it shouldn't
    // be a problem
    // {
...
Runnable* Runnable::getCurrentRunnable()
{
    ...
    /*if (gCurrentRunnableKeyCreated) //not multithread-safe but it shouldn't be
    a problem
    {
...
Runnable::Runnable( RunnableCreationOptions options )
    : mCurrentInfo( nullptr ),
      ...
      mTasks( options.schedulerOpts ), mOpts( std::move( options ) ),
      mOwnerThread( 0 ) // assume 0 is invalid thread id!!
{
    ...
    */
    //_addNewPool(); quitado hasta tener todo claro, pero parece que incluso
    // inicializar el pool cuesta mucho cuando es grande Create default timer.
...
void Runnable::postTask( std::shared_ptr<Process> process,
...
void Runnable::processTasks()
{
    ...
    /*if ( !mCurrentInfo )  //now initialize curranble info
    {
            ...
            if (ri == nullptr)
            {
                    ri = new RunnableInfo; //@todo ahora quedar� esta perdida de
    memoria TLS::setValue(gCurrentRunnableKey, ri);
            }
            mCurrentInfo = ri;
    }
    */

...
EventBase::EventBase( bool autoRelease, bool signaled )
    : mSignaled( signaled ), mAutoRelease( autoRelease )
{
}
...
void EventBase::reset() { mSignaled = false; }
void EventBase::_triggerActivation( bool sendToAll )
{
    if ( sendToAll )
    {
        for ( TProcessList::iterator i = mWaitingProcesses.begin();
              i != mWaitingProcesses.end(); ++i )
            ( *i )->wakeUp();
...
EventMTThreadSafePolicy::EventMTThreadSafePolicy( bool autoRelease,
                                                  bool signaled )
    : EventBase( autoRelease, signaled )
{
}
mel::tasking::EEventMTWaitCode
EventMTThreadSafePolicy::_wait( unsigned int msecs )
{
    ...
    if ( !EventBase::mSignaled )
    {
        ...
        EventBase::mWaitingProcesses.push_back(
            p ); // remember. not multithread-safe

        Process::ESwitchResult switchResult;
        if ( msecs == EVENTMT_WAIT_INFINITE )
            switchResult = ::mel::tasking::Process::sleepAndDo(
                [this]() { mCS.unlock(); } );
        else
            switchResult = ::mel::tasking::Process::waitAndDo(
                msecs, [this]() { mCS.unlock(); } );
        switch ( switchResult )
        {
        ...
        // remove process form list. It's safe to do it here because current
        // process is already waiting (now is returning) maybe other process do
        // wait on this events
        mCS.lock();
...
EventNoMTThreadSafePolicy::EventNoMTThreadSafePolicy( bool autoRelease,
                                                      bool signaled )
    : EventBase( autoRelease, signaled )
{
}
mel::tasking::EEventMTWaitCode
EventNoMTThreadSafePolicy::_wait( unsigned int msecs )
{
    EEventMTWaitCode result = EEventMTWaitCode::EVENTMT_WAIT_OK;
    if ( !EventBase::mSignaled )
    {

...
Process::Process( unsigned short capacity )
    : mPeriod( 0 ), mPauseReq( false ), mLastUpdateTime( 0 ),
      mOwnerProcessScheduler( 0 ), mState( EProcessState::PREPARED ),
      mWakeup( false )
...
Process::~Process( void ) { free( (void*)mStack ); }
...
void Process::reset()
{
    ...
    mLastUpdateTime = 0;
    // mLastTime = (unsigned
    // int)((mOwnerProcessScheduler!=NULL)?mOwnerProcessScheduler->getTimer()->getMilliseconds():0);
    // ??
    //	mPreviousTime = 0;
}
...
void Process::update( uint64_t msegs )
{
    ...
    //@todo mirar bien la diferencia entre estos dos. Supongo que este mLastTime
    // es diferente cuando hay cambios de contexto??? en principio no..
    //	mLastTime = (unsigned
    // int)mOwnerProcessScheduler->getTimer()->getMilliseconds(); mPreviousTime
    // =
    // msegs;
}
...
{
    return (
        unsigned int)( mOwnerProcessScheduler->getTimer()->getMilliseconds() -
...
void mel_tasking_resizeStack( MThreadAttributtes* process,
...
Process::ESwitchResult Process::switchProcess( bool v )
{
    ...
    if ( state == EProcessState::PREPARED_TO_DIE ||
         state == EProcessState::DEAD ||
         state == EProcessState::KILLING_WAITING_FOR_SCHEDULED )
    ...
    else
    {
        ...
        if ( std::current_exception() != nullptr )
            mel::text::warn(
                "Process::switchProcess. Doing context switch while pending "
                "exception can lead to crashes. You should avoid Process "
                "context switches on 'catch' contexts" );
#endif
        ...
        if ( p->mPauseReq &&
             !p->mWakeup ) // maybe a pause was  requested while switched
        {
            p->mPauseReq = false;
            sleep();
        }
        ...
        {
            p->setPeriod( currentPeriod ); // siempre restauramos por si acaso
                                           // se despertó por wakeup
...
mel::mpl::Tuple<TYPELIST( bool, Process* )> Process::_preWait()
{
    ...
    p->mSwitched = true; // needed to cheat that is already switched just in
                         // case is checked as a response of postSleep
    return mel::mpl::Tuple<TYPELIST( bool, Process* )>( prevSwitch, p.get() );
}
Process::ESwitchResult
Process::_postWait( uint64_t msegs,
                    mel::mpl::Tuple<TYPELIST( bool, Process* )> input )
{
    ESwitchResult result;
    auto p = input.get<1>();
    unsigned int currentPeriod = p->getPeriod();
...
Process::ESwitchResult Process::sleep() { return _postSleep( _preSleep() ); }
...
mel::mpl::Tuple<TYPELIST( int, Process*, unsigned int )> Process::_preSleep()
{
    ...
    {
        return mel::mpl::Tuple<TYPELIST( int, Process*, unsigned int )>(
            2, p.get(), 0 );
    }
    else if ( state == EProcessState::PREPARED_TO_DIE ||
              state == EProcessState::DEAD ||
              state == EProcessState::KILLING_WAITING_FOR_SCHEDULED )
    {
        return mel::mpl::Tuple<TYPELIST( int, Process*, unsigned int )>(
    ...
    p->mSwitched = true; // needed to cheat that is already switched just in
                         // case is checked as a response of postSleep
    return mel::mpl::Tuple<TYPELIST( int, Process*, unsigned int )>(
        prevSwitch ? 1 : 0, p.get(), currentPeriod );
}
Process::ESwitchResult Process::_postSleep(
    mel::mpl::Tuple<TYPELIST( int, Process*, unsigned int )> input )
{
    auto p = input.get<1>();
    if ( input.get<0>() == 2 ) // it hasn't any sense, but just in case this
...
void Process::wakeUp()
{
    //@todo termina verificar que está pausado y tal y poner estado anterior y
    // todo eso..
    if ( mSwitched || mState == EProcessState::ASLEEP )
    {
        ...
        setPeriod( 0 );
        //	mState = mPreviousState; //creo que esto no debe ser aqui!!! LO
        // HICE PORQUE EL SCHEDULER NO TENIA EN CUENTA SI SE HABÁI DESPERTADO
        mWakeup = true;
        onWakeUp();
    }
}
...
{
    // TODO esta funcion no est� un pijo bien, hay que revisarla junto con el
    // Process::execute
